...
- Talents v2 UI (overlay + HUD активок): `game.js` (`ensureTalentUI`, `updateTalentUI`, `updateStageAbilitySlots`) + контракт в `docs/ui_talents_v2.md`.
	...
	- Зелёный glow/pulse остаётся только на outgoing edges от прокачанных талантов; `active` edge больше не использует travelling dash/particle flow и вместо этого анимируется через pulse + jitter как энергия/ток: [src/ui/talentOverlayRenderer.js](../../../src/ui/talentOverlayRenderer.js#L91-L151), [style.css](../../../style.css#L2134-L2168).
...
- Разметка `#productionLineStorageModal` использует header `plStorage__header`: слева живёт help-button `#plStorageHelp`, в центре `#plStorageTitle`, справа `#plStorageClose` со skin `scModal__close`. Это canonical layout склада; не возвращать spacer-only header или plain-title + absolute-close схему: [index.html](../../../index.html#L214-L224), [style.css](../../../style.css#L6961-L6992).
...
## Cache-bust contract
- После post-merge UI-sync `index.html` держит единый cache-bust `?v=20260322-ui-postmerge` на `style.css`, `src/i18n/fallbackStrings.js`, `src/ui/talentOverlayRenderer.js`, `src/ui/supercomputerMenu.js`, `src/ui/productionLineUI.js` и `game.js`. Это браузерный контракт, который не даёт смешивать новый DOM/CSS/i18n/runtime с устаревшими static assets из cache: [index.html](../../../index.html#L10-L10), [index.html](../../../index.html#L587-L604), [index.html](../../../index.html#L629-L629).
