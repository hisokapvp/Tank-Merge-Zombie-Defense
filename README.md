...
- Язык хранится в `localStorage['lang']` и применяется при старте в порядке: `прочитать -> setLanguage -> отрендерить big menu`.
- Переключатель языка в big menu применяет язык глобально и мгновенно обновляет тексты big menu без перезагрузки страницы.
