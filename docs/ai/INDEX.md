...
- **Balance Editor analytics tab + zombie HP comparison**: `tools/balance-editor.html` документирован как repo-local tuning tool без build step: damage-points tab теперь считает `damage points / minute` только из live working copies `assets/tanks.json` и `assets/bullet.json` через `BalanceLab.Shared.getTankDamagePointMetrics()`, показывает provenance и явный diagnostic path вместо устаревшего runtime fallback, а также держит центрированную таблицу для level-by-level сверки source data; график `Танки vs Зомби` по-прежнему сравнивает tank shot damage против zombie HP с приоритетом явного `types[].Health` из `assets/zombies.json` над fallback-формулой `hpMul`.
