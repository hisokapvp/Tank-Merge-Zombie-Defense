...
- Фича (запрос игрока): уровень можно получить либо накопив нужное количество опыта, либо убив 500 000 зомби. Отдельная шкала не нужна — прогресс виден в тултипе полоски опыта: «Альтернативный вариант: Убить 500000 Зомби. Прогресс X / 500000».
- Ключевое требование: счётчик убийств для повышения уровня — **отдельный** от HUD-счётчика `state.kills` («Убито монстров» в терминале) и **сбрасывается после каждого полученного уровня**.
- Контракт: новый `supercomputer.levelKills` (per-level, не lifetime). Порог — `LEVEL_KILL_THRESHOLD = 500000` в `game.js`, пробрасывается в `levelFlow` как `killLevelThreshold`.
- `src/mechanics/levelFlow.js`: `grantXP()` отрефакторен на общий seam (`createLevelAggregate` / `consumeLevel` / `applyLevelRewards`), чтобы XP-путь и kill-путь выдавали идентичные награды (золото, очки улучшений, damage points, VFX, модалка, power-moments). Новый `grantKillProgress(amount)` копит `levelKills` и при достижении порога выдаёт уровень. Сброс — в 0 (остаток сверх порога **не** переносится) и происходит при **любом** полученном уровне, включая уровень по XP.
- `game.js`: kill-seam (`markZombieDying`) вызывает `grantKillProgress(1)` рядом с `grantXP(_killXp)`; `getComputerState()`/fallback `createInitialState()` сидят `levelKills: 0`.
- Persistence: `initialState.js` сидит `levelKills: 0`; `supercomputer.js ensureSupercomputerState()` нормализует поле; `worldReset.js` держит `levelKills` в partial-reset snapshot/restore (это прогрессия уровня, как `xp`).
- Тултип: `bootstrap.js` получил поддержку динамического провайдера — `data-ui-tooltip-provider="<name>"` резолвится через `Game.TooltipProviders[name]` (приоритет над статичным `data-ui-tooltip`); селекторы `pointerover`/`touchstart` расширены на provider-элементы. `game.js` регистрирует `TooltipProviders.xpLevel` и обновляет открытый тултип на месте в `updateProgressUI()` (иначе при неподвижном курсоре прогресс «замирал»).
- i18n: `levelAltKillTooltip` в `ru.json`, `en.json` и обоих блоках `fallbackStrings.js` (с `{kills}` и `{target}`).
- `Test/pack28/killLevelProgression.test.js`: 16 проверок — порог, накопление без касания `state.kills`, выдача уровня, сброс без переноса остатка, parity наград с XP-путём, сброс при XP-левелапе, maxLevel, нормализация битых значений, kill-seam, константа, initialState, partial reset, normalizer, i18n, provider-hook, bootstrap-селекторы. Wired в `ci/run_tests.sh`.
- `index.html`: entry token → `20260925-kill-level-progression` (179 маркеров, 0 mismatch, байт-безопасный bump: LF-only, без BOM, ASCII-only).
- Playwright (реальный браузер, `:8899`): тултип открывается на полоске опыта и обновляет прогресс без движения курсора (`345678 → 277777`).
- Урок: два источника одного события (XP и убийства) обязаны идти через **общий** seam наград, иначе модалка/VFX/очки разъезжаются; а быстро меняющийся текст тултипа нельзя писать в DOM каждый кадр — нужен provider-хук.

### Дроны исчезали при полном rack — overflow в подземный ангар + writer для `undergroundHangar`
- Баг (запрос игрока): когда все 9 слотов дронов заняты, новый дрон просто исчезал.
- Root cause (двойной): (1) `Drones.addDron()` при полном rack имел единственный overflow-путь — `tryAbsorbIncomingDroneIntoFullSlots()`, который поглощает дрона только при наличии дрона **того же уровня**; иначе возвращался `null` и награда терялась. (2) `state.undergroundHangar` **читался** в `restoreFullState()`/`applySavedProgress()`, но `serializeState()` его никогда не писал — даже успешно сложенный в ангар дрон исчезал после reload.
- Fix: `Game.UndergroundHangar.storeDrone(state, drone)` + `findFreeDroneCellIndex(state)` — канонический overflow-target. `Drones.addDron()` при полном rack кладёт дрона в свободную ячейку ангара (standby, `slotIndex: null`), legacy level-absorb остаётся вторым tier'ом, `null` — только когда полны оба ангара. `game.js addDron()` показывает toast `droneStoredUnderground`, когда дрон ушёл в ангар.
- Persistence: `storage.js serializeUndergroundHangar()` пишет 16 ячеек (`tank`/`drone` в одном shape, что и `cells`); `game.js __KNOWN_PAYLOAD_KEYS` + `assets/saveSchema.json` обновлены; `applySavedProgress()` восстанавливает ангар **безусловно** (payload без поля очищает его); `worldReset.js` держит `undergroundHangar` как `partial-preserve`.
- `Test/pack27/droneOverflowUndergroundHangar.test.js`: 15 проверок — overflow-семантика (store/absorb/null), writer parity через `saveSlot`/`loadSlot`, known-key, restore-контракт, partial reset, schema, i18n parity. Wired в `ci/run_tests.sh`.
- Урок: overflow-путь обязан быть **общим** для всех источников дрона (achievement / production box / shop / lottery / debug), а не дублироваться в каждом caller'е; и любой новый `state.*`-блок, который читает restore, обязан иметь writer в `serializeState()`.
...
- `src/systems/worldEventsRuntime.js`: вместо countdown-only API — полный снимок расписания. `getAttackWaveSnapshot()` возвращает `{ remainingSec, active, remainingActiveSec }`; `applyLoadedAttackWaveSnapshot(snapshot)` при `active: true` восстанавливает `attackEndAt = now + remainingActiveSec` и `currentAttackStartAt = now`, иначе — только countdown. `applyLoadedAttackWaveTiming()` остался countdown-only обёрткой. Остаток активной волны clamped `[0, attackDurationSec]`.
- `assets/saveSchema.json` + `src/persistence/serializedStateTypes.js`: два новых поля payload — `attackWaveActive` (boolean) и `attackWaveRemainingActiveSec` (number, `>= 0`).
- `src/persistence/storage.js`: `serializeAttackWaveRemainingSec()` заменён на `serializeAttackWaveSnapshot()` (live-first через `Game.getAttackWaveSnapshot()`, fallback на `state.attackWave*`); payload пишет все три поля.
- `game.js`: `GameApi.getAttackWaveSnapshot` + `applyLoadedAttackWaveSnapshot()`; `restoreFullState()` и legacy-путь `applySavedProgress()` применяют снимок. Восстановленная активная волна заново открывает achievement-эпизод (`beginNoRepairAttackWaveEpisode()` + `beginDefenseOrderEpisode()`), иначе `handleNoRepairAttackWaveTransition()` не вызывает `begin` при `wasAttackActive === true`. Все три поля добавлены в `__KNOWN_PAYLOAD_KEYS`.
- Docs: `docs/ai/SYSTEMS/worldEvents.md` (секция расписания волн переписана под снимок) и `docs/ai/SYSTEMS/save.md` (строка Payload Contract Map).
- `Test/pack17/attackWaveCountdownPersistence.test.js`: 25 проверок (было 17) — новые AWC-18…AWC-24 покрывают снимок активной волны, возобновление волны после загрузки, её завершение по расписанию, clamping и force-attack. Anchor `TUT-8X` в `Test/pack4/tutorial_first_run_runtime.test.js` обновлён под три поля.

### Обратный отсчёт до волны атаки сохраняется и продолжается после загрузки сейва
- Баг: полный `attackEverySec` (2 минуты) до первой волны выдавался не только при старте симуляции, но и при каждой загрузке обычного сейва. Игрок, сохранившийся посреди игры за 10 секунд до волны, после загрузки ждал 2 минуты заново.
- `src/systems/worldEventsRuntime.js` — три новых seam'а контроллера: `getAttackWaveRemainingSec()` (live countdown до `attackStartAt`, `null` если attack mode off/force), `applyLoadedAttackWaveTiming(remainingSec)` (продолжить расписание + сбросить фантомное окно атаки прошлой сессии), `scheduleFirstAttackWaveAfterRestart()` (единственный owner «полного интервала после старта симуляции»).
- `assets/saveSchema.json` + `src/persistence/serializedStateTypes.js`: новое поле payload `attackWaveRemainingSec` (`number|null`, clamp `[0, attackEverySec]`). Персистится **относительный** остаток, потому что `worldEventsState.attackStartAt` — абсолютное sim-время текущей страницы.
- `src/persistence/storage.js`: writer `serializeAttackWaveRemainingSec(state)` — live-first через `Game.getAttackWaveRemainingSec()`, fallback на `state.attackWaveRemainingSec`.
- `game.js`: `GameApi.getAttackWaveRemainingSec`, `applyLoadedAttackWaveTiming()` и `scheduleFirstAttackWaveAfterRestart()`; `resetWorldEventsRuntimeForNewGame()` теперь делегирует планирование контроллеру. `restoreFullState()` при `forceFenceRuntimeResetOnLoad` (= «Перезапуск симуляции» / critical-сейв «Сохранить прогресс и выйти») игнорирует поле и даёт полный интервал, иначе зовёт `applyLoadedAttackWaveTiming(saved.attackWaveRemainingSec)`. Legacy-progress путь (`applySavedProgress`) читает то же поле. Поле добавлено в `__KNOWN_PAYLOAD_KEYS`.
- `index.html`: entry token поднят до `20260923-attack-wave-countdown`, все 179 `?v=` тегов синхронизированы (включая `src/persistence/storage.js`, `src/persistence/serializedStateTypes.js`, `src/systems/worldEventsRuntime.js`).
- Docs: `docs/ai/SYSTEMS/worldEvents.md` (новая секция «Расписание волн: старт симуляции vs продолжение сейва») и `docs/ai/SYSTEMS/save.md` (строка Payload Contract Map + anchor TUT-8X).
- `Test/pack17/attackWaveCountdownPersistence.test.js` (новый, 17 проверок) + регистрация в `ci/run_tests.sh`; anchor-кейс `TUT-8X` в `Test/pack4/tutorial_first_run_runtime.test.js`. Тест проверен на pre-fix источниках — падает (AWC-9…AWC-16).

### Коллизия декора гаснет импульсами вместо телепорта застрявших зомби
- Баг: зомби массово застревали в элементах карты из `assets/decor.json` (все 4 фрейма — `isWall: true`). Прежний fail-safe (`maybeTeleportZombieNearFence()` + `findZombieFenceFailSafeTeleport()`) срабатывал редко (25 сек стагнации, ±5 px допуск, ≤20 px от blocker'а) и телепортировал зомби к внешней стороне ограды, что маскировало застревание, но не решало его.
- `src/mechanics/decorCollision.js` (новый модуль `Game.DecorCollisionPulse`): чистый таймлайн окна прохода. Каждые `periodSec` (10 сек) коллизия ВСЕХ `isWall`-декораций выключается на случайные `offDurationMinSec..offDurationMaxSec` (1–2 сек). Следующее окно планируется от момента закрытия, поэтому период не «сжимается». `normalizeConfig()` валидирует authoring-значения (окно никогда не ≥ половины периода), `enabled: false` — полный kill-switch.
- `game.js`: `stepDecorCollisionPulse(dt)` вызывается один раз за кадр в начале `stepZombies()` — пауза симуляции (menu/tab/critical) автоматически замораживает окно, т.к. `effDt` не тикает. `resolveZombieWallMove()` и `isZombieDecorBlockedAt()` через `isDecorCollisionSuppressed()` перестают учитывать `state.wallDecors`, пока окно открыто; `initDecors()` вызывает `resetDecorCollisionPulse()`. Добавлены `resolveDecorCollisionPulseConfig()` / `ensureDecorCollisionPulseState()` / `getDecorCollisionPulseDebugInfo()`.
- Удалено (безопасно): функции `findZombieFenceFailSafeTeleport()` / `maybeTeleportZombieNearFence()`, поля зомби `failSafeTeleported` / `failSafeDecorSinceSec` / `failSafeDecorAnchorX` / `failSafeDecorAnchorY`, вызов телепорта из `stepZombies()` и файл `Test/pack8/zombieFailSafeTeleport.test.js`. Обычный `_unstickTimer`-nudge (4 сек / ≥2 px) сохранён как мягкий assist: он больше не телепортирует зомби.
- `assets/decor.json`: новая секция `collisionPulse: { enabled: true, periodSec: 10, offDurationMinSec: 1, offDurationMaxSec: 2 }` + обновлён `_readme`. `BAL.decorCollisionPulse` (объект или `null`) добавлен в `game.js` как runtime-override в том же стиле, что `BAL.decorCount` / `BAL.decorNoSpawnZones`. `src/render/spriteLoaders.js` прокидывает блок в `DecorSprites.config.collisionPulse` как сырой объект — нормализация живёт в `game.js`, чтобы loader не зависел от порядка `<script>`.
- `index.html`: entry token поднят до `20260923-decor-collision-pulse` и синхронизированы все `?v=` теги (включая новые `src/mechanics/decorCollision.js`, `src/ui/adService.js`, `src/systems/talents/talentsV2.js`, `src/i18n/fallbackStrings.js`, `src/yandex/yandexSdk.js`) — теперь один токен на весь entry-surface.
- `Test/pack8/decorCollisionPulse.test.js` (новый, 12 проверок) + регистрация в `ci/run_tests.sh` вместо удалённого pack8 teleport-теста: дефолты и clamping конфига, границы окна (блокировка вплоть до 10-й секунды, открытие ровно на границе, закрытие через 1–2 сек, повтор через полный период), масштабирование окна random-фактором, kill-switch, устойчивость к `null`/`NaN`/отрицательному dt, `resetState()`, wiring в оба decor-seam'а, полное отсутствие teleport-символов, parity decor.json ↔ loader ↔ index.html ↔ CI.
- Docs: `docs/ai/SYSTEMS/combat.md` (секция unstick переписана: pulse вместо телепорта), `docs/ai/SYSTEMS/assets.md` (контракт `collisionPulse`), `docs/ai/INDEX.md`, `docs/ai/GAME_JS_MAP.md`.
- Verification: `node Test/pack8/decorCollisionPulse.test.js` → 12 passed, 0 failed; `node Test/tests.js` → без регрессий; `node --check`/`new Function` OK для `game.js`, `src/mechanics/decorCollision.js`, `src/render/spriteLoaders.js`, `Test/pack8/decorCollisionPulse.test.js`; `assets/decor.json` → валидный JSON.

### Модалка «новый уровень танка» срабатывает при merge в любом ангаре
- Баг: popup «ОТКРЫТ НОВЫЙ УРОВЕНЬ ТАНКА!» вызывался только из `performMerge` (main↔main), поэтому merge двух танков, сделанный переносом танка из подземного ангара (`_performUndergroundMerge` / `_performCrossHangarMerge`), создавал танк нового уровня молча — без модалки.
- `game.js`: введён единый seam `_notifyMergeNewTankLevel(oldMaxLevel, lvl, fxContext)`, который владеет показом `Game.MergePopup.show(lvl)` и выбором SFX (`mergeNewMaxLevel` vs `levelUp`) вместе с `playMergeFx`. Все три пути merge теперь зовут его после `recordTankLevel(lvl)`, а `oldMaxLevel` снимается до него. Дублирующая локальная переменная `isNewMaxLevelMergePopup` из `performMerge` удалена.
- `fxContext` опционален: подземные ячейки не имеют геометрии доски, поэтому underground/cross-hangar ветки передают `null` и `resolveMergeFxPosition` уходит в fallback `center`; main-ветка по-прежнему передаёт `{resultCellIndex, resultTankId}`. Дедупликация показа осталась в самом popup (`seenMergeLevels`), поэтому повторный merge того же уровня не открывает модалку дважды.
- Слои перепроверены: `#mergePopupModal` — child `<body>` (`z-index:100`), `#undergroundHangarOverlay` (`z-index:500`) лежит внутри `.stage` (`z-index:9999`), вложенного в `.layout` (`z-index:1`). Именно `.layout` создаёт stacking context, поэтому popup рисуется поверх открытой модалки ангара и его кнопки кликабельны. Изменений в CSS не требовалось.
- `index.html`: entry token поднят до `20260923-merge-popup-all-hangars`; синхронизированы `?v=` для `src/ui/adService.js`, `src/systems/talents/talentsV2.js`, `src/i18n/fallbackStrings.js` и `src/yandex/yandexSdk.js` (pack15 CB-2/CB-4 parity).
- Docs: `docs/ai/SYSTEMS/ui.md` (секция Merge popup: инвариант места вызова, контракт `_notifyMergeNewTankLevel`, слои, расширенный QA-чеклист) и `docs/ai/SYSTEMS/audio.md` (SFX-выбор централизован в seam).
- `Test/pack16/mergePopupAllHangars.test.js` (новый, 12 проверок) + регистрация в `ci/run_tests.sh`: guard на наличие seam, вызов из всех трёх merge-путей, порядок `oldMaxLevel → recordTankLevel → notify`, отсутствие дублирующего `MergePopup.show` в ветках, опциональность `fxContext`. Тест проверен на pre-fix `game.js` — падает (11 из 12), на post-fix — 12/12.
- Verification: `node Test/tests.js` → 102 passed; `node Test/pack16/mergePopupAllHangars.test.js` → 12 passed; `node Test/pack15/crateIntervalAndRewardedAd.test.js` → 35 passed; Playwright-проверка в браузере: popup появляется при main-, underground- и cross-hangar-merge, рисуется поверх открытой модалки подземного ангара, кнопка «В бой!» кликабельна. Pre-existing (не связанные с задачей): `Test/pack6/mergePopupPreview.test.js` MPV-2…MPV-5, `Test/pack1/newGamePopupReset.test.js` T5-13, `Test/pack10/phaser_migration_phase4.test.js` IH-6E/IH-6F и trailing whitespace в `src/render/canvasRoot.js` — красные и без этой правки.

### «Обновить моментально» в древе улучшений запускает rewarded-рекламу Яндекс.Игр
- `src/ui/adService.js`: `#talentResetCooldownModalRefresh` добавлен третьим элементом в `AD_GATED_SELECTORS = ['#crateGet', '#plConfirmYes', '#talentResetCooldownModalRefresh']` — новая площадка подключена selector-only правкой, без нового кода в гейте. Raw-клик блокируется, вызывается `requestRewardedAd()`, и только при `success === true` переиздаётся ровно один синтетический клик. Политика сбоя не менялась: technical failure → fail-open, досрочное закрытие → fail-closed; in-flight защита `sdkAdInFlight` и watchdog `120000ms` общие.
- `src/systems/talents/talentsV2.js`: новый публичный API `clearRespecCooldown({ nowMs })` — снимает только 2-часовой cooldown-гейт. `resetCount` **намеренно сохраняется**: иначе ценовая лестница респека (500$ × множитель) фармилась бы рекламой. Возвращает `{ ok, reason?, resetCount, cooldownEndsAtMs, cooldownRemainingMs }`, состояние персистится через `persistSave()`. Если кулдаун уже истёк — `ok: false, reason: 'no_cooldown'` (идемпотентно, без награды).
- `game.js`: `handleTalentResetCooldownRefreshNow()` превращён из stub-toast в реальный обработчик — вызывает `clearRespecCooldown()`, синкает `state.player.talentsV2.respec`, закрывает cooldown-модалку и заново входит в обычный платный флоу `requestResetAllTalents()` (confirm `#resetTalentsModal` → списание монет через `tryRespec`). Стоимость сброса по-прежнему берётся с игрока, реклама даёт только «пропуск таймера». Tooltip CTA переключён на новый ключ.
- i18n: `talentResetCooldownRefreshStub` («заглушка ещё не подключена») заменён на честный `talentResetCooldownRefreshAdTooltip` («Посмотрите рекламу, чтобы сбросить улучшения не дожидаясь окончания таймера» / «Watch an ad to reset upgrades without waiting for the timer to end») синхронно в `ru.json`, `en.json` и `fallbackStrings.js` (оба locale).
- `index.html`: entry token поднят до `20260923-talent-reset-ad-refresh`; синхронизированы `?v=` для затронутых HTML-loaded контрактов — `src/ui/adService.js`, `src/systems/talents/talentsV2.js`, `src/i18n/fallbackStrings.js` и `src/yandex/yandexSdk.js` (последний — по pack15 CB-2 parity).
- `Test/pack15/crateIntervalAndRewardedAd.test.js`: CB-1 переведён с хардкода устаревшего токена на parity-проверку (`resolve('style.css')` / `resolve('game.js')`) — тест больше не краснеет после release-пасса. Добавлены CB-4 (talentsV2/fallbackStrings parity) и Section 6 TRR-1…TRR-5 (площадка в списке, кнопка в HTML, `clearRespecCooldown` + сохранение `resetCount`, отсутствие stub-toast в handler'е, i18n parity и retirement старого ключа) — 35 проверок вместо 29.
- Verification: `node Test/pack15/...` → 35 passed, 0 failed; `node Test/tests.js` → 102 passed; `node ci/check_talent_helpers.cjs` → OK (17 wired / 3 TODO); `node --check` OK для всех изменённых JS. Pre-existing: `Test/pack10/phaser_migration_phase4.test.js` IH-6E/IH-6F и trailing whitespace в `src/render/canvasRoot.js` — красные независимо от этой задачи.
...
- `index.html`: в `.crateModal__content` добавлена красная кнопка `#crateDecline` («Отказаться») и вложенный confirm `#crateDeclineConfirm` (`role="alertdialog"`) с текстом `#crateDeclineConfirmText` и кнопками `#crateDeclineCancel` («Отмена») / `#crateDeclineConfirmYes` («Продолжить»). Entry token поднят до `20260917-crate-decline-button`, локальные query bump'ы — для `fallbackStrings.js`, `modals.js`, `CrateRewardScene.js`.
- `style.css`: `.crateModal__declineBtn` — красная палитра (`#6d2119 → #3b0f0a`, `border rgba(255,86,66,.7)`, `color #ffd7cf`) с hover/focus-visible состояниями; `.crateModal__declineConfirm` / `.crateModal__declineBox` / `.crateModal__declineActions` — вложенный confirm в той же панели. **Важно:** цветовые правила живут после `.btnSecondary`, т.к. первоначальная раскладка выше `.btn` проигрывала cascade равной специфичности (background/border оставались generic-серыми).
- `game.js`: `ui.crateDecline*` refs; `isCrateDeclineConfirmOpen()` / `openCrateDeclineConfirm()` / `closeCrateDeclineConfirm()` / `declineCrateReward()`; wiring кнопок; backdrop-close теперь no-op пока confirm открыт; `closeCrateModal()` снимает confirm первым (порядок a11y-стека).
- **Награда не выдаётся:** `declineCrateReward()` зеркалит cleanup claim-пути (`state.crate = null`, `nextCrateAt = nowSec() + BAL.crateIntervalSec`) и намеренно не вызывает `grantCrateTank()`/`grantUndergroundCrateTank()`, `recordTankLevel()` и `recordBonusBoxOpened()` — отказанный бокс не считается открытым, box_hunter family не растёт.
- `src/ui/modals.js`: `openCrateDeclineConfirm` / `closeCrateDeclineConfirm` / `isCrateDeclineConfirmOpen` + синхронизация copy и сброс confirm при каждом открытии модалки.
- i18n parity: `crateDecline`, `crateDeclineConfirmText`, `crateDeclineCancel`, `crateDeclineContinue` добавлены одновременно в `src/i18n/ru.json`, `src/i18n/en.json` и оба блока `src/i18n/fallbackStrings.js`.
- Phaser parity: `CrateRewardScene` — красная decline-кнопка `BTN_DECLINE`, confirm sub-view `_setConfirmOpen()` / `_buildConfirmView()`, `onDecline` callback; confirm схлопывается **после** `_setAllVisible(true)`, иначе подтверждение появлялось бы вместе с боксом.
- Verification: `node Test/tests.js` → 97 passed, 0 failed; `phaser_migration_phase3b` → 33 passed; `node --check` OK для всех изменённых JS + оба i18n JSON парсятся; Playwright — красная кнопка `linear-gradient(rgb(109,33,25), rgb(59,15,10))` / `border rgba(255,86,66,.7)`; клик → confirm с «Отмена»/«Продолжить»; «Отмена» возвращает к боксу; «Продолжить» убирает бокс, танк/монеты/`bonusBoxesOpenedCount` не изменились, confirm сбрасывается при re-open; backdrop-клик при открытом confirm не закрывает модалку; Escape закрывает confirm, второй Escape — модалку; EN-локаль отдаёт Decline/Cancel/Continue; claim-путь по-прежнему выдаёт танк (регрессия не задета); mobile 420px — кнопка 347px внутри панели 400px. Единственный trailing-whitespace warning — pre-existing `src/render/canvasRoot.js` (вне scope).

## 2026-09-16

### HUD «Текущая волна: X» в терминале суперкомпьютера
- `index.html`: под `#simResetsWrap` добавлена отдельная панель `#currentWaveWrap` (класс `.xpPanel.hudPanel` — тот же shell, что и у «Перезагрузка симуляции»), внутри `#currentWaveText` с дефолтом «Текущая волна: 0». Токен cache-bust поднят до `20260916-current-wave-hud`.
- `game.js`: `ensureProgressUI()` получил fallback-создание `#currentWaveWrap` (для сборок со старым HTML); `updateProgressUI()` читает canonical per-run счётчик `state.stats.currentWaveCount` и пишет через тот же шаблон `t('supercomputerCurrentWaveInfo')` `.replace('{count}', N)`, что и `simResetsText`.
- `game.js`: новый helper `incrementCurrentWaveCounter()` — единственная точка инкремента; вызывается ровно один раз из `finalizeNoRepairAttackWaveEpisode()` (переход attack active → inactive), рядом с `completeAttackEpisodeAchievementProgress()`. `beginNoRepairAttackWaveEpisode()` счётчик не трогает, поэтому значение растёт **только после окончания волны**.
- **Per-run семантика** (user choice A): `stats.currentWaveCount` намеренно **не** входит в `takeProgressSnapshot()`/`restoreProgressSnapshot()` в `src/core/worldReset.js`, поэтому «Перезагрузка симуляции» и New Game начинают с 0, а save/load внутри одного run прогресс волн сохраняет. Отличие от lifetime `attackWavesCompletedCount`, который partial reset не сбрасывает.
- Persistence: `createInitialState()` seed'ит `stats.currentWaveCount = 0`; `serializeState()` пишет нормализованное поле; `getSerializedAchievementStats()`/`applySavedAchievementStats()` в `game.js` включают его в payload и restore (нет legacy mirror — fresh-start поле, паттерн `bonusBoxesOpenedCount`).
- i18n parity: `supercomputerCurrentWaveInfo` добавлен одновременно в `src/i18n/ru.json` («Текущая волна: {count}»), `src/i18n/en.json` («Current wave: {count}») и оба блока `src/i18n/fallbackStrings.js`.
- `Test/tests.js`: новый регрессионный тест `T-ACH-3` — проверяет HTML-shell и порядок панелей, fallback-создание, seed/serialize поля, локацию инкремента (в finalize, не в begin), отсутствие `currentWaveCount` в progress snapshot и i18n parity.
- `docs/ai/SYSTEMS/save.md` + `src/persistence/serializedStateTypes.js`: зафиксирован owner tag `progression`, per-run контракт и обязательность отсутствия поля в progress snapshot.
- Verification: `node Test/tests.js` → 97 passed, 0 failed (`T-ACH-3` ✓); `node --check` OK для `game.js`, `storage.js`, `initialState.js`, `fallbackStrings.js`; оба i18n JSON парсятся; Playwright на локальном сервере `:8899` — панель появляется прямо под `simResetsWrap`, computed style идентичен (`border: 1px solid rgb(255,0,255)`, тот же `linear-gradient(135deg, rgba(20,10,35,.9) …)`, `boxShadow rgba(255,0,255,.2)`, `padding 8px 10px`, `fontSize 12px`, `color rgb(74,246,38)`, `textTransform uppercase`, ширина `192px`); debug force-attack цикл дал инкремент `0 → 1`, текст стал «Текущая волна: 1»; EN-локаль показала «Current wave: 13» при живом переключении языка; `takeProgressSnapshot()` подтверждённо не содержит `stats`. Единственный trailing-whitespace warning — pre-existing `src/render/canvasRoot.js` (вне scope).

### Суперкомпьютер больше не уезжает за забор при загрузке сейва в другом размере окна
- **Root cause**: `state.supercomputer.x` / `.y` / `.offsetY` — runtime-derived от текущего `state.boardRect`, который зависит от размера viewport. Но `restoreFullState()` и `applySavedProgress()` мержили весь сохранённый объект `supercomputer` через `Object.assign`, затирая только что вычисленные координаты устаревшими (fullscreen). Путь загрузки из big-menu (`BigMenuRuntime.startFromBigMenu` → `restoreFullState`) не вызывает `initBoard()` после restore, поэтому координаты никто не пересчитывал — суперкомпьютер оставался на fullscreen-позиции и оказывался за забором.
- `game.js`: новый канонический helper `repositionSupercomputerFromBoard()` — единственный source-of-truth для `sc.x`/`sc.y`/`sc.offsetY` (центр boardRect по X, низ boardRect + `offsetY` по Y). Fail-soft: если `state.boardRect` ещё не инициализирован (pre-boot restore), координаты не трогаются, а следующий `initBoard()` их пересчитает.
- `game.js`: `initBoard()` теперь делегирует в helper вместо инлайновой формулы (`hangarCenterX`/`hangarBottomY` удалены).
- `game.js`: `restoreFullState()` и `applySavedProgress()` вызывают `repositionSupercomputerFromBoard()` сразу после merge payload. Удалён старый хак `_scPrevX`/`_scPrevY` («сохранить предыдущие координаты, если в payload нули») — он лечил симптом (телепорт в 0,0), а не причину.
- `game.js`: новый helper `syncSupercomputerDependentLayout(sc)` — обновляет всё, что позиционируется относительно суперкомпьютера (production line render layout, HUD-кнопка, level-driven sprite sync). Вызывается из `initBoard()` и из обоих restore-путей, чтобы зависимый UI не оставался на устаревших координатах.
- `Test/pack13/supercomputerViewportPlacement.test.js`: новый регрессионный пак `SVP-1..SVP-7` — проверяет helper, делегирование из `initBoard()`, reposition после merge в обоих restore-путях, fail-soft guard, resync зависимого layout и удаление старого хака. Итог: 7 passed, 0 failed.
- Verification: `node --check game.js` OK; `node Test/tests.js` → 96 passed, 0 failed; `Test/pack13/supercomputerViewportPlacement.test.js` → 7/0; Playwright smoke на локальном сервере: save в 1920×1080 → reload в 1280×720 → load slot → `sc.x/y` совпадают с `boardRect`-формулой, суперкомпьютер визуально внутри забора; resize 1280×720 → 1920×1080 → 676×858 → 929×917 сохраняет инвариант.

### Сохранение/загрузка: чипы, вставленные в ячейки ангара, больше не исчезают
- **Root cause**: установленные в ячейки чипы не живут в runtime `state` — они лежат в module-owned grid `_cells` внутри `src/ui/hangarChipsUI.js` (`Game.HangarChipsUI.getCells()`/`setCells()`), и `serializeState()` их вообще не писал. При этом `playerChips` (инвентарь) сериализовался корректно, из-за чего потеря выглядела как баг инвентаря. Загрузка любого сейва (включая `restoreFullState`/`applySavedProgress`/critical restart) поднимала пустой grid.
- `src/persistence/storage.js`: новые хелперы `serializeHangarChipSlot()`/`serializeHangarChipSlots()`/`serializeHangarCells()`; в payload добавлено поле `hangarCells`. Пишется только persisted subset (`{ id, redSlots, yellowSlots }` + `{ chipId, modIds, sourceComboKey, rotation, level }`); derived-поля (`activeModifiers`, `uiState`, `tankId`) не сохраняются. Источник — live `Game.HangarChipsUI.getCells()` (зеркалит паттерн `playerChips`, поэтому любой save-путь, включая slot-save с raw `state`, захватывает актуальный grid), fallback — `state.hangarCells`. Возвращает `null`, если grid недоступен.
- `src/ui/hangarChipsUI.js`: `setCells()` переписан из «просто присвоить `_cells`» в fail-soft нормализацию — приводит grid к канонической 16-ячеечной форме через `createHangarCellsState()` (fallback `createFallbackCellsTemplate()`), проверяет `modIds`, нормализует `rotation` в `0..2` и `level >= 1`, отбрасывает битые slots, пересчитывает `activeModifiers`/`uiState` через `calculateActiveModifiers()` и перерисовывает overlay, если он открыт. `setCells([])` очищает grid — легаси-сейв без `hangarCells` не утаскивает чипы из предыдущей сессии.
- `game.js`: `restoreFullState()` и `applySavedProgress()` вызывают `HangarChipsUI.setCells(Array.isArray(...hangarCells) ? ... : [])` сразу после реконсиляции `_unlockedTechs` (чтобы `resolveLatestTechModId` успел развернуть legacy modIds в unlocked-тиры при пересчёте модификаторов). Вызов безусловный — явная ветка очистки для легаси-сейвов. `hangarCells` добавлен в `__KNOWN_PAYLOAD_KEYS`.
- Инвариант New Game сохранён: `resetPlayerInventory({ reason: 'new_game' })` по-прежнему пересоздаёт пустой grid (`_cells = null` + `ensureCells()`), поэтому требование «после Новой игры у игрока не остаётся чипов» не регрессирует; partial reset grid не трогает и чипы сохраняет.
- `assets/saveSchema.json`: добавлено optional-поле `hangarCells` (`["array","null"]`, items с `id` + `redSlots`/`yellowSlots`); легаси-сейвы без поля валидны.
- `src/persistence/serializedStateTypes.js` + `docs/ai/SYSTEMS/save.md`: синхронизированы typedef `SerializedState.hangarCells` и строка Payload Contract Map (owner tag `hangar`).
- `Test/pack12/hangarCellsPersistence.test.js`: новый регрессионный пак `HCP-1..HCP-10` — round-trip через реальный write-path `saveSlot`/`loadSlot`, проверка persisted subset, fallback на `state.hangarCells`, `null` при отсутствии grid, восстановление slots + пересчёт `activeModifiers`, fail-soft на битых slots, очистка grid, безусловные вызовы `setCells` в обоих restore-путях, сохранение New Game reset и наличие ключа в `__KNOWN_PAYLOAD_KEYS`. Итог: 10 passed, 0 failed.
- Verification: `node --check` OK для `game.js`, `src/persistence/storage.js`, `src/ui/hangarChipsUI.js`; `node Test/tests.js` → 96 passed, 0 failed; `Test/pack12/hangarCellsPersistence.test.js` → 10/0; `Test/pack1/newGamePopupReset.test.js` → 16/1 (pre-existing `T5-13`); `Test/pack8/chipEffectVisualToggle.test.js` → 15/0; `Test/pack11/fenceDamagePersistence.test.js` → 6/0. Baseline `T5-13` и pack4 (TUT-2B/TUT-10F/TUT-12) падали до правок и не являются регрессией.

### New Game: сброс чипов, вставленных в ячейки ангара
- **Root cause**: установленные в ячейки чипы живут не в `state`, а в module-owned grid `_cells` внутри `src/ui/hangarChipsUI.js` (16 ячеек, создаются `Game.HangarChips.createHangarCellsState()`). Grid не сериализуется и не входит в `createInitialState()`, поэтому после «Новая игра» ранее вставленные чипы продолжали работать до перезагрузки страницы.
- `src/ui/hangarChipsUI.js`: `resetPlayerInventory(meta)` теперь дополнительно делает `_cells = null` + `ensureCells()` (пересоздание пустого grid), сбрасывает `_selectedCell = 0`, `_chipFilter = 'all'`, `_selectedSlot`/`_visualSelectedSlot`/`_activeSlotActions`, отменяет залипший drag через `_cancelDragsHook` и (если overlay инициализирован) вызывает `render()`: [src/ui/hangarChipsUI.js](../../src/ui/hangarChipsUI.js).
- `init()` регистрирует свой scoped canceller drag'а в module-level `_cancelDragsHook`, потому что `_slotDragging` и `_cancelAllDrags` живут внутри `init()` и недоступны reset-seam'у: [src/ui/hangarChipsUI.js](../../src/ui/hangarChipsUI.js).
- `Test/pack1/newGamePopupReset.test.js`: тест `T5-17` расширен проверками `_cells = null` + `ensureCells()`, сброса sticky-состояния, регистрации `_cancelDragsHook` и сохранения вызова seam только на new_game-ветке. Итог пака: 16 passed, 1 pre-existing failed (`T5-13`).
- `docs/ai/SYSTEMS/save.md`: зафиксировано, что установленные в ячейки чипы — module-owned in-memory grid и обязаны сбрасываться reset-seam'ом.
- Verification: `node --check src/ui/hangarChipsUI.js` OK; `node Test/tests.js` → 96 passed, 0 failed; `Test/test_shop_apply_bundle.js` → 10/0; `Test/pack11/productionLineUnlocks.test.js` → 16/0; baseline `T5-13` и `pack4` (TUT-2B/TUT-10F/TUT-12) падали до правок.

### New Game: гарантированный старт с 0 чипов без перезагрузки страницы
- **Root cause**: `resetGameState({ reason: 'new_game' })` пересоздаёт `state` через `createInitialState()`, поэтому `state.playerChips` становится `[]`, но `HangarChipsUI` держит собственные копии инвентаря (`_playerChipsFallback`/canonical owner, `_playerFragments`, `_siliconDust`, `_techFeedProgress`, `_techStudying`) и не является чисто derived view — они переживали New Game до перезагрузки браузера.
- `src/ui/hangarChipsUI.js`: новый публичный seam `resetPlayerInventory(meta)`. Очищает chips через canonical writer `setPlayerChips([], { reason: 'new_game' })` (один emit `playerChips.changed`), обнуляет `_playerFragments`, `_siliconDust`, `_techFeedProgress`, `_techAccelDustSelected`, останавливает `_stopTechStudyTimer()` + сбрасывает `_techStudying`, затем вызывает `resetTransientUiState()`. Экспортирован в public API: [src/ui/hangarChipsUI.js](../../src/ui/hangarChipsUI.js).
- `game.js`: в ветке `reason === 'new_game'` внутри `resetGameState()` вызывается `Game.HangarChipsUI.resetPlayerInventory({ reason: 'new_game' })` рядом с существующим сбросом unlocked modifier techs. Partial reset (`reason === 'reset'`) не затронут — инвентарь по-прежнему восстанавливается из snapshot: [game.js](../../game.js#L13244-L13256).
- `Test/pack1/newGamePopupReset.test.js`: новый регрессионный тест `T5-17` — проверяет и seam, и его полный набор очищаемых ресурсов, и вызов только на new_game-ветке, и сохранение chip-snapshot при partial reset. Итог пака: 16 passed, 1 pre-existing failed (`T5-13`).
- `docs/ai/SYSTEMS/save.md`: зафиксирован контракт «New Game обязан очищать инвентарь чипов» с указанием seam, веток и теста.
- Verification: `node --check` OK для `game.js` и `src/ui/hangarChipsUI.js`; `node Test/tests.js` → 96 passed, 0 failed; baseline `T5-13` и `pack4` (TUT-2B/TUT-10F/TUT-12) падали до правок и не являются регрессией.

### Чипы: крафт из фрагментов II/III уровня и синхронизация после прокачки модификаций
- **Root cause #1 — неработающая установка скрафченного чипа**: `assembleChip()` сопоставлял комбинацию с пулом по «живым» modIds. Любой фрагмент тех-тира (15–30) давал `sourceComboKey` вида `2-16-18`, в пуле (только базовые моды 1–14) такого ключа нет, поэтому чип возвращался c `chipId: -1`. Все install-пути резолвят чип через `getChipById()`, который для `-1` возвращает `null` — чип «создавался», но не вставлялся ни в один слот.
- **Root cause #2 — «переворачивание» чипа после прокачки**: `normalizeRedPlacement()` / `normalizeYellowPlacement()` сортировали modIds по живому значению. После unlock тех-тира (2 → 17) мод переезжал в конец сортировки, вершины`A/B/C` (и `innerA/innerB`) переставлялись, и `checkRedMatch` / `checkYellowMatch` переставали совпадать — парные чипы с двумя общими модификациями больше не синхронизировались.
- `src/mechanics/hangarChips.js`: новые `MOD_BASE_BY_MOD` / `resolveBaseTechModId(modId)` / `sortModIdsByBase(modIds)` — fold тех-тиров на базовый мод (1–14). `assembleChip()` теперь валидирует и ищет чип по базовому ключу, а возвращает реальный `chipId`/`sourceComboKey` пула c живыми (тировыми) `modIds`; невалидные комбинации (все одинаковые / 2+ special) по-прежнему отклоняются. `normalizeRedPlacement()` / `normalizeYellowPlacement()` сортируют через `sortModIdsByBase()`, поэтому раскладка вершин стабильна между тирами. Новый `resolveChipDefForModIds(modIds)` для резолва чипа по modIds без chipId: [src/mechanics/hangarChips.js](../../src/mechanics/hangarChips.js#L140-L195), [src/mechanics/hangarChips.js](../../src/mechanics/hangarChips.js#L313-L362), [src/mechanics/hangarChips.js](../../src/mechanics/hangarChips.js#L670-L730).
- `src/ui/hangarChipsUI.js`: новый `_resolveChipDefForEntry(h, record, chipId)` — сначала пул, затем base-key fallback; используется в `installChipAction()`, drag-install path, `removeChipAction()`, `autoInstall()` и debug-хелпере, чтобы чип никогда не терялся и не уничтожался из-за устаревшего chipId. Новые `_healPlayerChipEntry()` / `_healPlayerChips()` вызываются из `addPlayerChip()` и `setPlayerChips()`, поэтому легаси-записи с `chipId: -1` само-лечатся на load без schema bump. `_resolveCraftResultChipDef()` переписан как defensive seam через `resolveChipDefForModIds()`. `_getChipMatchTargetLabels()` больше не гейтит match-хинты на успешный pool-lookup. `restoreSavedSlots()` резолвит chipId по modIds, если id неизвестен: [src/ui/hangarChipsUI.js](../../src/ui/hangarChipsUI.js#L1506-L1532), [src/ui/hangarChipsUI.js](../../src/ui/hangarChipsUI.js#L2215-L2245), [src/ui/hangarChipsUI.js](../../src/ui/hangarChipsUI.js#L5093-L5115).
- `Test/pack12/chipCraftAndTechMatch.test.js`: новый регрессионный пак `CFS-1..CFS-12` — крафт из тиров II/III резолвится в реальный chipId и находится через `getChipById`, невалидные комбинации отклоняются, базовый крафт без дрейфа, red/yellow-пары остаются в синхроне после unlock II и III, mixed-tier слоты совпадают, ложных совпадений нет, `setPlayerChips()` лечит `chipId: -1`, HTR-паритет сохранён.
- `ci/run_tests.sh`: зарегистрированы ранее не подключённые паки `pack11` (`metaHoarderUnlocks`, `productionLineUnlocks`) и `pack12` (`hangarCellsPersistence`, `chipCraftAndTechMatch`).
- `docs/ai/HANGAR_CHIPS_UI_MAP.md`: зафиксированы три новых инварианта (идентификация чипа по базовому составу, порядок вершин по базовому modId, обязательный fallback-резолв в UI).
- Verification: `node Test/pack12/chipCraftAndTechMatch.test.js` → 12 passed, 0 failed; `node Test/pack12/hangarCellsPersistence.test.js` → 10 passed; `node Test/pack8/hangarChipTechResolution.test.js` → 2 passed; `ci/check_style.sh` (trailing whitespace) OK. Pre-existing падения `pack1/newGamePopupReset`, `pack8/offlineProgress`, `pack8/balanceOptimizerLocks`, `pack8/balanceCurveRepair` подтверждены через `git stash` как не связанные с этими правками.

## 2026-09-16
- **Равномерное распределение танков по треку — docs pass** (`src/mechanics/trackDistribution.js` [new], `game.js`, `index.html`, `Test/tests.js`, `docs/ai/GAME_JS_MAP.md`, `docs/ai/index.yaml`, `docs/ai/PROJECT_MAP.md`, `docs/ai/ARCHITECTURE.md`, `docs/ai/SYSTEMS/combat.md`)
  - Новый модуль `src/mechanics/trackDistribution.js` → `Game.TrackDistribution`: `TWO_PI`, `isOnTrackTank(tank)`, `countOnTrackTanks(cells)`, `computeSlotPlacement(cells, cellIndex, out?)` (возвращает `{index, count}`, `index = -1` если ячейка не на треке; порядок слотов детерминирован по возрастанию `cell.i`), `computeSlotOffsetRad(index, count)` = `index / count * 2π`, `normalizePhase(rad)`. Zero-alloc контракт: необязательный `out`-объект, hot-path не аллоцирует.
  - `game.js`: module-level `_sharedTrackPhase` + `getSharedTrackPhase()` / `seedSharedTrackPhase(value)` / `ensureSharedTrackPhase()` — единая фаза трека для всех танков; `ensure` сидируется `orbitPhase` первого уже стоящего на треке танка, иначе `0`: [game.js](../../game.js#L10581-L10603).
  - `stepTanks(dt)` больше не применяет per-level `balSpeedMul` к угловой скорости трека. Вместо этого один раз за кадр считается `sharedAngularSpeed = BAL.tankOrbitSpeed * speedMult() * mods.orbitSpeedMul * activeSpeed * resolveTrackOrbitMul()`, единая фаза продвигается один раз и записывается во все `onTrack`-ячейки (`cell.orbitPhase = _sharedTrackPhase`): [game.js](../../game.js#L10609-L10617). `balAtkSpeedMul` для `attackSpeedMul` сохранён.
  - Новые хелперы: `resolveTrackOrbitMul()` — максимальный `orbit` из `getBarrageMul()` по всем танкам на треке («Шквал» ускоряет весь трек целиком) [L11215](../../game.js#L11215); `resolveTrackSlotOffsetRad(cell)` — угловое смещение слота через `Game.TrackDistribution` с переиспользуемым scratch-объектом `_trackSlotScratch` [L11204-L11212](../../game.js#L11204-L11212).
  - `tankOrbitState(cell, timeSec)`: для onTrack-танка угол = `cell.orbitPhase + resolveTrackSlotOffsetRad(cell)`; старая формула `(cell.i / (rows*cols)) * 2π` перемещена в legacy/hangar-ветку (танк не на треке) и больше не влияет на трек: [game.js](../../game.js#L11232-L11262).
  - Drag-release на трек присваивает `from.orbitPhase = ensureSharedTrackPhase()` (танк присоединяется к общей фазе) [L15273](../../game.js#L15273); `_sharedTrackPhase = null` при full reset [L13210](../../game.js#L13210) и при restore сейва с нормализацией к фазе первого onTrack-танка [L7646-L7662](../../game.js#L7646-L7662); обновлён устаревший комментарий про `barrageMul.orbit` в `fireTankProjectile` [L11037-L11045](../../game.js#L11037-L11045).
  - `index.html`: подключён `<script src="src/mechanics/trackDistribution.js?v=20260916-track-uniform-distribution">` сразу после `trackQuery.js` [L896-L897](../../index.html#L896-L897); токен `game.EntryAssetVersion` (cache-bust для `style.css` и `game.js`) поднят с `20260527-solo-pipeline-yandex-vk-batch1-v5` на `20260916-track-uniform-distribution` [L174](../../index.html#L174).
  - `Test/tests.js`: новая группа `T10` (метки `T10-1`..`T10-9`) на `Game.TrackDistribution` — равный шаг `360°/N`, независимость от уровня, детерминированный порядок по `cell.i`, `index = -1` вне трека, `normalizePhase`, zero-alloc `out`-объект, пустой трек: [Test/tests.js](../../Test/tests.js#L80-L172). Итог прогона: 96 passed, 0 failed.
  - **Контракт поведения**: танки на треке делят полный круг на равные сектора `360°/N` независимо от уровня танка и от того, в каких ячейках ангара они находятся; скорость вращения едина для всех танков. Save-schema НЕ менялась (`cell.orbitPhase` сохраняется как раньше), i18n-строк не добавлялось.
  - Документация only: runtime-код, тесты и config в этом docs-pass не изменялись.
