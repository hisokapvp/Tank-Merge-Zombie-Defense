...
## Post-merge update (2026-03-29)
- `src/ui/supercomputerMenu.js` теперь документируется как canonical owner SC-family shell contract: `showSharedHelpModal()` / `syncSharedHelpButtonCopy()` держат общий help-shell для talents/hangar/tank-wall/storage, `shouldUseFullscreenShell()` остаётся единственным fullscreen helper для coarse-pointer / `< 1280px` именно для talents/hangar/tank-wall, а `applySharedTalentModalClass()` владеет talents header action-row и видимым порядком `help -> close`; fullscreen talents path deliberately держит outer shell и mounts в `overflow:hidden`, оставляя реальный scroll-containment за `#supercomputerTalentsView` и `.talentTreeBody`, тогда как embedded `.modalClose` сохраняется только как parity-skin/hit-area слой: [src/ui/supercomputerMenu.js](../../../src/ui/supercomputerMenu.js#L150-L185), [src/ui/supercomputerMenu.js](../../../src/ui/supercomputerMenu.js#L422-L469), [src/ui/supercomputerMenu.js](../../../src/ui/supercomputerMenu.js#L553-L690), [style.css](../../../style.css#L2682-L2720), [style.css](../../../style.css#L7995-L8110).
- Production Storage shell теперь зафиксирован как `default / expanded / mobile-fit` centered-panel contract: `_getResponsiveShellMode()` выбирает `mobile-fit` ниже `1000px`, `expanded` для coarse-pointer или `< 1280px`, `style.css` root token block рядом с modal-family vars хранит `--pl-storage-*` default/expanded/mobile families, `.plStorage--expanded` даёт safe-area wrapper, а `.plStorage__panel--expanded` / `.plStorage__panel--mobileFit` переключают capped width/height и scale tokens вместо fullscreen-stretch поведения: [src/ui/productionLineUI.js](../../../src/ui/productionLineUI.js#L94-L120), [index.html](../../../index.html#L255-L262), [style.css](../../../style.css#L19-L82), [style.css](../../../style.css#L8113-L8210).
...
- Top-right terminal теперь документируется с отдельным sub-1200 contract: root `--ui-terminal-width` / `--ui-terminal-expand-size` задают базовый shell, а media override `< 1200px` ужимает ширину `.stageUiRight`, padding/header spacing `.terminal-panel` и размеры HUD controls без локального разрыва с master `--ui-scale`: [style.css](../../../style.css#L85-L86), [style.css](../../../style.css#L8304-L8320).
...
- Разметка `#productionLineStorageModal` использует header `plStorage__header`: title остаётся центрированным, но help и close теперь собраны в правый wrapper `.plStorage__headerActions`; help-button `#plStorageHelp` сидит рядом с `#plStorageClose`, а заголовок получает увеличенный `padding-top`, чтобы визуально не съезжать под action-row. Не возвращать старую схему с отдельной left-help кнопкой: [index.html](../../../index.html#L255-L262), [style.css](../../../style.css#L7420-L7453).
- Для narrow/mobile storage path modal использует split responsive contract: `_getResponsiveShellMode()` переключает `default / expanded / mobile-fit`, `.plStorage--expanded` добавляет safe-area wrapper для coarse-pointer / `< 1280px`, а сама `.plStorage__panel--expanded` остаётся centered bounded shell с capped width/height; `.plStorage--mobileFit` включает отдельный `< 1000px` fit-mode с собственными token families. Реализация deliberate остаётся в runtime class toggles + root `--pl-storage-*` vars, а не в ad-hoc inline/mobile overrides: [src/ui/productionLineUI.js](../../../src/ui/productionLineUI.js#L94-L120), [style.css](../../../style.css#L19-L82), [style.css](../../../style.css#L8113-L8210), [index.html](../../../index.html#L255-L270).
...
- CSS scale contract покрывает static DOM shells и dynamic DOM families: terminal, HUD, tutorial, help/confirm/tooltips/notifications, modal action rows, underground hangar sizing vars и storage token families `default/expanded/mobile-fit`; локальный hangar cap снят, потому что `--modsHangarScale` теперь напрямую читает `--ui-scale`: [style.css](../../../style.css#L15-L82), [style.css](../../../style.css#L1664-L1692), [style.css](../../../style.css#L3774-L3888), [style.css](../../../style.css#L4242-L4268), [style.css](../../../style.css#L7612-L7726), [style.css](../../../style.css#L8113-L8185).
- Top-right terminal не живёт на отдельном ad-hoc mobile scale-path: root vars `--ui-terminal-width` / `--ui-terminal-expand-size` остаются частью этого же scale contract, а media override `< 1200px` лишь локально сжимает shell width, panel padding, header chrome и stage-ability spacing, сохраняя ownership у общего `--ui-scale`: [style.css](../../../style.css#L85-L86), [style.css](../../../style.css#L8304-L8320).
