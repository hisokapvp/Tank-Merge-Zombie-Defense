...
function ensureAchievementsState(){
  if (AchievementsApi && AchievementsApi.ensureState) {
    ...
    if (!achievementsState.rewarded || typeof achievementsState.rewarded !== 'object') achievementsState.rewarded = {};
  ...
  if (!Number.isFinite(state.achievements.totalManualFenceRepairs)) state.achievements.totalManualFenceRepairs = 0;
...
function processAchievementProgress(progressType, deltaCount){
  ...
  if (AchievementsApi && AchievementsApi.addProgress) {
    ...
    else if (type === 'manualFenceRepairs') ach.totalManualFenceRepairs += count;
...
function completeNoRepairAttackWaveAchievementProgress(){
  let unlocked = [];
  if (AchievementsApi && typeof AchievementsApi.recordNoRepairAttackWaveSuccess === 'function') {
    unlocked = AchievementsApi.recordNoRepairAttackWaveSuccess(state) || [];
  } else {
    const ach = ensureAchievementsState();
    const nextValue = clampDevInt(ach.totalNoRepairAttackWaveStreak + 1);
    ach.totalNoRepairAttackWaveStreak = nextValue;
    if (!state.stats || typeof state.stats !== 'object') state.stats = {};
...
function resetNoRepairAttackWaveAchievementProgress(){
  ...
  const ach = ensureAchievementsState();
  ach.totalNoRepairAttackWaveStreak = 0;
  if (!state.stats || typeof state.stats !== 'object') state.stats = {};
  state.stats.noRepairAttackWaveStreakCount = 0;
}

function invalidateNoRepairAttackWaveEpisode(){
  if (!noRepairAttackWaveRuntime.activeEpisodeKey || noRepairAttackWaveRuntime.invalidated) return false;
  noRepairAttackWaveRuntime.invalidated = true;
  resetNoRepairAttackWaveAchievementProgress();
  return true;
}
...
function checkPerfectFenceWave(){
...
function checkHangarMasterAchievement(){
  const level = computeHangarMasterLevel();
  if (level <= 0) return;
  let currentProgress = 0;
  if (AchievementsApi && typeof AchievementsApi.getProgressValue === 'function') {
    currentProgress = AchievementsApi.getProgressValue(state, 'hangarMasterLevel');
  }
  if (level > currentProgress) {
    processAchievementProgress('hangarMasterLevel', level - currentProgress);
  }
}

function findFenceSegmentByIdForNoRepairTracking(segmentId){
  if (segmentId == null || !Array.isArray(state.fenceSegments)) return null;
...
function hasActiveRepairDrone(){
  if (!Array.isArray(state.drones)) return false;
  for (let i = 0; i < state.drones.length; i++) {
    if (state.drones[i] && state.drones[i].mode === 'repair') return true;
  }
  return false;
}

function captureFenceHpSnapshotForNoRepairTracking(){
  if (!noRepairAttackWaveRuntime.activeEpisodeKey || noRepairAttackWaveRuntime.invalidated) return null;
  if (!hasActiveRepairDrone() || !Array.isArray(state.fenceSegments) || !state.fenceSegments.length) return null;
  const hpById = {};
  let hasTrackedSegments = false;
  for (let i = 0; i < state.fenceSegments.length; i++) {
    const seg = state.fenceSegments[i];
    if (!seg || seg.id == null || !Number.isFinite(seg.hp) || !Number.isFinite(seg.maxHp)) continue;
    if (seg.hp >= seg.maxHp) continue;
    hpById[seg.id] = seg.hp;
    hasTrackedSegments = true;
  }
  return hasTrackedSegments ? hpById : null;
}

function invalidateNoRepairAttackWaveOnDroneRepair(hpSnapshot){
  if (!hpSnapshot || !noRepairAttackWaveRuntime.activeEpisodeKey || noRepairAttackWaveRuntime.invalidated) return false;
  const ids = Object.keys(hpSnapshot);
  for (let i = 0; i < ids.length; i++) {
    const segmentId = ids[i];
    const seg = findFenceSegmentByIdForNoRepairTracking(segmentId);
    if (!seg || !Number.isFinite(seg.hp)) continue;
    if (seg.hp > hpSnapshot[segmentId]) {
      return invalidateNoRepairAttackWaveEpisode();
    }
  }
  return false;
}
...
function tryBuyTank(opts){
  ...
  if (bought) {
    processAchievementProgress('purchases', 1);
...
function levelGoldReward(level){
  if (ProgressionApi && ProgressionApi.levelGoldReward) {
    return ProgressionApi.levelGoldReward(level, BAL, LevelRewardConfig);
  }
...
function getLevelFlowController(){
  ...
  return LevelFlowApi.createLevelFlow({
    ...
    windowObj: window,
    levelRewardConfig: LevelRewardConfig,
  });
}
...
function saveProgress(){
  ...
  try{
    ...
    localStorage.setItem('progress', JSON.stringify({
      ...
      achievements: ach,
      stats: getSerializedAchievementStats(),
...
function restoreFullState(saved){
  ...
  if (saved.achievements && typeof saved.achievements === 'object') {
    ...
    ach.unlocked = saved.achievements.unlocked && typeof saved.achievements.unlocked === 'object'
      ...
      : ach.unlocked;
    ach.rewarded = saved.achievements.rewarded && typeof saved.achievements.rewarded === 'object'
    ...
    ach.totalMerges = Number.isFinite(saved.achievements.totalMerges)
      ...
      : ach.totalMerges;
    ach.totalManualFenceRepairs = Number.isFinite(saved.achievements.totalManualFenceRepairs)
      ? Math.max(0, Math.floor(saved.achievements.totalManualFenceRepairs))
      : ach.totalManualFenceRepairs;
    ach.totalModifierTechUnlocks = Number.isFinite(saved.achievements.totalModifierTechUnlocks)
      ? Math.max(0, Math.floor(saved.achievements.totalModifierTechUnlocks))
      : ach.totalModifierTechUnlocks;
    ach.totalDroneAcquisitions = Number.isFinite(saved.achievements.totalDroneAcquisitions)
      ? Math.max(0, Math.floor(saved.achievements.totalDroneAcquisitions))
      : ach.totalDroneAcquisitions;
  ...
  applySavedAchievementStats(saved.stats);
  ...
  /* Reconstruct _unlockedTechs from completedModifierTechs so tier-2 prerequisites resolve */
  {
    ...
    if (_HC && typeof _HC.setUnlockedTechs === 'function') {
      ...
      /* Reconcile from actual inventory/cells: picks up techs unlocked before tracking */
      if (typeof _HC.reconcileUnlockedTechsFromData === 'function') {
        _HC.reconcileUnlockedTechsFromData(
          state.playerChips || [],
          state.cells || [],
          (window.Game.HangarChipsUI && typeof window.Game.HangarChipsUI.getPlayerFragments === 'function')
            ? window.Game.HangarChipsUI.getPlayerFragments() : []
        );
        /* Sync any newly-discovered techs back to completedModifierTechs */
        const reconciledTechs = _HC.getUnlockedTechs();
        if (reconciledTechs && typeof reconciledTechs === 'object') {
          const rKeys = Object.keys(reconciledTechs);
          let rChanged = false;
          for (let ri = 0; ri < rKeys.length; ri++) {
            if (reconciledTechs[rKeys[ri]] && !ach.completedModifierTechs[rKeys[ri]]) {
              ach.completedModifierTechs[rKeys[ri]] = true;
              rChanged = true;
            }
          }
          if (rChanged) {
            const totalTracked = Object.keys(ach.completedModifierTechs).length;
            ach.totalModifierTechUnlocks = Math.max(ach.totalModifierTechUnlocks || 0, totalTracked);
            if (state.stats && typeof state.stats === 'object') {
              state.stats.modifierTechUnlocksCount = Math.max(state.stats.modifierTechUnlocksCount || 0, totalTracked);
            }
          }
        }
      }
...
function applySavedProgress(data){
  ...
  if (achievements && typeof achievements === 'object') {
    ...
    ach.rewarded = achievements.rewarded && typeof achievements.rewarded === 'object' ? { ...achievements.rewarded } : ach.rewarded;
    ...
    ach.totalManualFenceRepairs = Number.isFinite(achievements.totalManualFenceRepairs) ? Math.max(0, Math.floor(achievements.totalManualFenceRepairs)) : ach.totalManualFenceRepairs;
  ...
  applySavedAchievementStats(data.stats);
...
function grantAchievementReward(achievementId){
  ...
  if (entry.type === 'composite' && Array.isArray(entry.items)) {
    for (var ci = 0; ci < entry.items.length; ci++) {
      var sub = entry.items[ci];
      if (sub.type === 'damagePoints') updateDamagePointsUI();
      if (sub.type === 'upgradePoints' && isTalentsV2Ready()) {
        var tApi = getTalentsV2Api();
        if (tApi && typeof tApi.setFreePoints === 'function') {
          tApi.setFreePoints(state.player.talentsV2.freePoints);
          syncPlayerTalentsV2FromApi();
        }
      }
...
function reconcileAchievementRewards(achievementIds){
  ...
  for (let i = 0; i < achievementIds.length; i++) {
    grantAchievementReward(achievementIds[i]);
  }
}

function reconcileAchievementRewardsForUnlocked(){
  const ach = ensureAchievementsState();
  if (!ach || !ach.unlocked || typeof ach.unlocked !== 'object') return;
  const defs = getAchievementDefinitions();
  for (let i = 0; i < defs.length; i++) {
    if (!ach.unlocked[defs[i].id]) continue;
    grantAchievementReward(defs[i].id);
  }
}

function onModifierTechnologyUnlocked(modId){
  const techId = clampDevInt(Number(modId));
  if (techId <= 0) return [];
  let unlocked = [];
  let usedFallbackRecalc = false;
  if (AchievementsApi && typeof AchievementsApi.recordModifierTechUnlock === 'function') {
    unlocked = AchievementsApi.recordModifierTechUnlock(state, techId) || [];
  } else {
    const ach = ensureAchievementsState();
    if (!ach.completedModifierTechs || typeof ach.completedModifierTechs !== 'object') ach.completedModifierTechs = {};
    if (ach.completedModifierTechs[String(techId)]) return [];
    ach.completedModifierTechs[String(techId)] = true;
    ach.totalModifierTechUnlocks = clampDevInt(ach.totalModifierTechUnlocks + 1);
    if (state.stats && typeof state.stats === 'object') {
      state.stats.modifierTechUnlocksCount = ach.totalModifierTechUnlocks;
    }
    unlocked = recalculateAchievementsAndQueuePopups();
    usedFallbackRecalc = true;
  }
...
GameApi.onModifierTechnologyUnlocked = onModifierTechnologyUnlocked;
...
const ZOMBIE_DEBUFF_OVERLAY_SIZE_PX = 14;
const ZOMBIE_DEBUFF_OVERLAY_STEP_PX = 16;
const ZOMBIE_DEBUFF_OVERLAY_LIMIT = 3;
const ZOMBIE_DEBUFF_STATUS_PRIORITIES = {
  stun: 100,
  slow: 90,
  mark: 80,
  acid: 70,
...
function getTalentModNumber(mods, key, aliases, fallback){
  let value = null;
  if (mods && Number.isFinite(mods[key])) value = Number(mods[key]);
  if (!Number.isFinite(value) && Array.isArray(aliases)) {
    for (let i = 0; i < aliases.length; i++) {
      const aliasKey = aliases[i];
      if (mods && Number.isFinite(mods[aliasKey])) {
        value = Number(mods[aliasKey]);
        break;
      }
    }
  }
...
function computeTalentStatusFill(untilMs, durationMs, nowMs){
  const until = Number.isFinite(untilMs) ? untilMs : 0;
...
function pushZombieDebuffOverlayCandidate(count, iconKey, priority, fill01){
  ...
  let insertAt = count;
  if (insertAt > ZOMBIE_DEBUFF_OVERLAY_LIMIT - 1) insertAt = ZOMBIE_DEBUFF_OVERLAY_LIMIT - 1;
...
function drawScaledDebuffExpiryOverlay(ctx, centerX, centerY, sizePx, fill01){
  if (!ctx) return;
  const fill = clamp(Number.isFinite(fill01) ? fill01 : 0, 0, 1);
  if (fill <= 0.0001) return;
  const size = Math.max(1, Number.isFinite(sizePx) ? sizePx : ZOMBIE_DEBUFF_OVERLAY_SIZE_PX);
  const left = centerX - size * 0.5;
  const top = centerY - size * 0.5;
  const startAngle = -Math.PI * 0.5;
  const endAngle = startAngle + Math.PI * 2 * fill;
  const dotRadius = Math.max(1, (size / ZOMBIE_DEBUFF_OVERLAY_SIZE_PX) * 1.4);
  ctx.save();
  ctx.beginPath();
  ...
  ctx.fill();
  ctx.beginPath();
  ctx.arc(centerX, centerY, dotRadius, 0, Math.PI * 2, false);
  ctx.fillStyle = 'rgba(255,255,255,0.78)';
  ctx.fill();
  ctx.restore();
}

function drawScaledZombieDebuffOverlays(ctx, talentsApi, zombies, nowMs, debuffIconScale, debuffIconOpacity){
  ...
  const mods = talentsApi.getMods();
  const stunDurationMs = getTalentModNumber(mods, 'stunOnWallHitDurationMs', [], 0);
  const slowDurationMs = getTalentModNumber(mods, 'ccMicroSlowDurationMs', [], 0);
  const markDurationMs = getTalentModNumber(mods, 'markDurationMs', [], 0);
  const acidDurationMs = getTalentModNumber(mods, 'acidDotDurationMs', [], 0);
  const convertDurationMs = getTalentModNumber(mods, 'convertToDotDurationMs', [], 0);
...
function draw(){
  ...
  if ((!_RR || _RR.isLegacy('talentStatusIcons')) && isTalentsV2Ready()) {
    ...
    if (talentsApi && typeof talentsApi.renderStatusIcons === 'function') {
      ...
      drawScaledZombieDebuffOverlays(
        ctx,
        talentsApi,
        state.zombies,
        statusNowMs,
        _zSprites ? _zSprites.debuffIconScale : 1,
        _zSprites ? _zSprites.debuffIconOpacity : 1
      );
    }
  }
...
function resolveTankAuraVisual(cellIndex, level) {
  ...
  if (auraSprite) auraSprite.variant = variant;
  return auraSprite;
}
...
function drawTankAuraSprite(x, y, aura){
  ...
  const variant = Number.isFinite(aura && aura.variant) ? aura.variant : 0;
  const variantStyle = variant >= 1 && variant <= 3 ? AuraSpriteStyleByVariant[variant] : null;
  let sx, sy;
...
async function boot(){
  ...
  bootPromise = (async function runBootFlow() {
    ...
    // ── Load levelreward.json for data-driven level rewards ──
    try {
      const lrRes = await fetch('assets/levelreward.json', { cache: 'no-store' });
      if (lrRes.ok) {
        LevelRewardConfig = await lrRes.json();
