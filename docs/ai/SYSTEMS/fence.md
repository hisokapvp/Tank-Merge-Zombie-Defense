...
- Базовое правило: до входа внутрь зомби выбирает ближайший breach только на своей текущей стороне (`top/right/bottom/left`, side-lock).
- Зомби использует центр ближайшего breach как промежуточную цель входа.
- После входа внутрь базы (breach crossed / `z.breached=true`) зомби продолжает маршрут к `supercomputer`.
- После входа внутрь базы (breach crossed / `z.breached=true`) дополнительный внутренний радиальный clamp не применяется.
- Если на своей стороне бричей нет, поведение остаётся прежним: зомби атакует fence-сегменты и не «знает» о бричах на других сторонах.

## Collision rule in breach hole

- Fence-block/collision отключается внутри `holeAabb` активного брича текущей стороны (side-limited pass до фактического входа).
