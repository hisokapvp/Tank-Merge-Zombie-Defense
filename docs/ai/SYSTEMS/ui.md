...
- **Лейаут панели**: `chipCraftLayout` содержит `chipCraftPreview` (drop-зона + кнопки режимов) и `chipCraftInventory` (инвентарь + нижняя панель действий). Кнопки режимов (`chipCraftModeRow`) расположены внутри `chipCraftPreview`, **после** drop-зоны (`chipCraftDropZone`).
