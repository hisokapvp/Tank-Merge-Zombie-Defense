...
- Обычные close-кнопки `.crateModal__close`, `.levelModal__close`, `.modalClose`, `.lessonProgress__close` используют одну orange 44×44 pseudo-element X-геометрию; `scModal__close`, `#talentOverlay .modalClose` и `.modalClose.scModal__close` — green SC-ветку того же паттерна: [style.css](../../../style.css#L1042-L1095), [style.css](../../../style.css#L1340-L1426), [style.css](../../../style.css#L1863-L1957), [style.css](../../../style.css#L3121-L3172).
- Геометрия крестика теперь центрируется только через `top/left:50%` + `translate(-50%, -50%) rotate(...)`; hover/active/focus не должны сдвигать сам `X` или hit-area через `transform`. Допустимы только визуальные эффекты (`filter`, `border-color`, `box-shadow`): [style.css](../../../style.css#L1066-L1095), [style.css](../../../style.css#L1364-L1393), [style.css](../../../style.css#L1885-L1914), [style.css](../../../style.css#L3143-L3172).
...
- **Создать чип (assemble)**: игрок кладёт 3 фрагмента в фиксированные слоты → получает целый чип.
	...
	- **Превью результата**: `_previewAssembleResult()` и `renderChipCraftPanel()` раскладывают 3 слота + стрелку + результат в одну горизонтальную строку `.chipCraftSlotRow--withResult`: [src/ui/hangarChipsUI.js](../../../src/ui/hangarChipsUI.js#L2103-L2207), [src/ui/hangarChipsUI.js](../../../src/ui/hangarChipsUI.js#L2517-L2544), [style.css](../../../style.css#L4480-L4493).
...
- Панель `workshopPanelChipRecycle` рендерит тот же `renderChipCraftPanel()`, но в режиме recycle-panel. Внутри неё всегда есть nested sub-tabs `chipRecycleTabDust` / `chipRecycleTabDisassemble`; canonical state хранится в `_chipRecycleSubTab`: [index.html](../../../index.html#L292-L301), [src/ui/hangarChipsUI.js](../../../src/ui/hangarChipsUI.js#L685-L697), [src/ui/hangarChipsUI.js](../../../src/ui/hangarChipsUI.js#L2418-L2441), [src/ui/hangarChipsUI.js](../../../src/ui/hangarChipsUI.js#L2679-L2692).
- **Распылить (Pulverize / Dust view)**: recycle-tab `dust` делает layout одноколоночным (`chipCraftLayout--singleCol`), скрывает preview/right-column и показывает инвентарь одновременно для больших чипов и фрагментов. Нижняя панель содержит только `Подтвердить`, `Отменить` и live-summary по пыли: [src/ui/hangarChipsUI.js](../../../src/ui/hangarChipsUI.js#L2418-L2529), [style.css](../../../style.css#L3749-L3756), [style.css](../../../style.css#L4471-L4474), [style.css](../../../style.css#L4683-L4712).
	...
	- `Отменить` внутри dust view очищает только `_dustSelected` и оставляет пользователя в той же recycle-подвкладке: [src/ui/hangarChipsUI.js](../../../src/ui/hangarChipsUI.js#L2707-L2715).
- **Разобрать (Disassemble view)**: recycle-tab `disassemble` показывает только целые чипы, сохраняет preview/action-column и использует динамический массив `_craftSlots` для массовой разборки; `_executeCraftAction()` обрабатывает все занятые chip-slots в цикле: [src/ui/hangarChipsUI.js](../../../src/ui/hangarChipsUI.js#L2537-L2668), [src/ui/hangarChipsUI.js](../../../src/ui/hangarChipsUI.js#L2801-L2933).
...
- Кнопка «Ускорить процесс открытия»: показывает модальное окно (`_showTechAccelModal`) с единым grid для кремниевой пыли, больших чипов и фрагментов. Ставки считаются через `_getTechAccelRates(modId)`: для 2ч технологий `dust/chip/fragment = 2/20/6`, для 5ч — `1/10/1`; все ресурсы делят общий cap `95%`, а элементы, не влезающие в остаток, получают `techAccelChip--disabled` + badge `Лимит`. Grid ресурсов теперь оборачивается в `techAccelGridWrap` (rounded border + scrollbar skin), а summary строка живёт под dust-row, а не над списком ресурсов. Пыль выбирается отдельным `+/-` stepper, нижняя строка всегда показывает `доступно / выбрано`, а `apply` использует тот же selection-state без отдельного скрытого пересчёта: [src/ui/hangarChipsUI.js](../../../src/ui/hangarChipsUI.js#L1909-L1988), [src/ui/hangarChipsUI.js](../../../src/ui/hangarChipsUI.js#L1990-L2047), [style.css](../../../style.css#L4222-L4315).
...
- Для UI-правок мастерской и production storage синхронно поддерживать ключи `workshopTabChipRecycle` и `plStorageTitle` в `ru.json`, `en.json` и `fallbackStrings.js`: [src/i18n/ru.json](../../../src/i18n/ru.json#L401-L433), [src/i18n/en.json](../../../src/i18n/en.json#L401-L433), [src/i18n/fallbackStrings.js](../../../src/i18n/fallbackStrings.js#L236-L275), [src/i18n/fallbackStrings.js](../../../src/i18n/fallbackStrings.js#L534-L575).
