...
> Агент-ориентировано. Обновлён: 2026-03-21.
> `style.css` — CSS-монолит проекта (~5243 строки) и один из главных hotspot-файлов.
...
- Chip craft / future chip / storage modal → [style.css](../../style.css#L4335-L5180).
...
- Talent tree visual contract держится в `style.css`: `talentTreeContainer` задаёт `--talent-node-icon-size: 40px`, уменьшенный `--talent-row-gap`, базовые рёбра остаются явно серыми, а anchor линий считается от центра `.talentNodeIcon`; green glow/pulse разрешён только для `ready/active` edges от уже прокачанных талантов, причём `active` edge использует pulse + jitter без travelling dash/particle flow. Applied-node в `#talentOverlay` остаётся зелёным, а `maxed` добавляет orange overlay только на внутренний icon-shell: [style.css](../../style.css#L2076-L2108), [style.css](../../style.css#L2134-L2168), [style.css](../../style.css#L2181-L2241), [style.css](../../style.css#L2275-L2307), [style.css](../../style.css#L2325-L2350), [src/ui/talentOverlayRenderer.js](../../src/ui/talentOverlayRenderer.js#L9-L21), [src/ui/talentOverlayRenderer.js](../../src/ui/talentOverlayRenderer.js#L91-L151).
...
- Storage modal использует отдельный header-grid `.plStorage__header` (`44px / 1fr / 44px`) со spacer слева, центрированным title и close справа; это устраняет лишний пустой хвост и выравнивает заголовок со слотами: [style.css](../../style.css#L5156-L5180).
...
| Chip craft / reagent row / recycle single-column layout / production line storage modal | [style.css](../../style.css#L4335-L5180) | Inventory, slots, result preview, dust-only layout, future chip frame, storage header/grid |
...
- [style.css](../../style.css#L2134-L2168) — ready/active edge pulse + jitter contract без travelling dash flow.
...
- [style.css](../../style.css#L5156-L5180) — production line storage header/grid alignment.
