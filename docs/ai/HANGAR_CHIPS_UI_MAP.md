...
- Assemble craft перед шансом < 100% обязан показывать confirm modal, а после любого roll — result modal. Шанс считается как `CRAFT_CHANCE_BASE_PCT (75) + _craftReagentDust * CRAFT_CHANCE_PER_DUST_PCT (3)`, cap `CRAFT_REAGENT_DUST_MAX = 5` ед. (максимум 90%). При провале сжигается только один случайный выбранный фрагмент и reagent dust, если он был добавлен; остальные выбранные фрагменты остаются в инвентаре: [src/ui/hangarChipsUI.js](../../src/ui/hangarChipsUI.js#L1746-L1758), [src/ui/hangarChipsUI.js](../../src/ui/hangarChipsUI.js#L2484-L2528), [src/ui/hangarChipsUI.js](../../src/ui/hangarChipsUI.js#L3102-L3194), [style.css](../../style.css#L4310-L4379).
...
- **Чип идентифицируется по БАЗОВОМУ мод-составу, а не по живому тех-тиру.** `assembleChip()` сворачивает modIds 15–30 на их базовые 1–14 через `resolveBaseTechModId()` и ищет чип в пуле по базовому ключу, возвращая реальный `chipId` с живыми (тировыми) `modIds`. До фикса крафт из фрагментов II/III уровня давал `chipId: -1`, и чип нельзя было поставить ни в один слот, потому что все install-пути резолвят чип через `getChipById()`: [src/mechanics/hangarChips.js](../../src/mechanics/hangarChips.js#L693-L725).
- **Порядок вершин чипа (`A/B/C`, `innerA/innerB`) определяется базовым modId.** `normalizeRedPlacement()` и `normalizeYellowPlacement()` сортируют через `sortModIdsByBase()`, поэтому unlock тех-тира (2 → 17) не меняет раскладку вершин и парные чипы продолжают синхронизироваться. Сортировка по живому modId «переворачивала» чип и ломала `checkRedMatch` / `checkYellowMatch`: [src/mechanics/hangarChips.js](../../src/mechanics/hangarChips.js#L313-L362).
- **Резолв чипа в UI не должен падать на `getChipById`.** `_resolveChipDefForEntry()` сначала пробует пул, затем `resolveChipDefForModIds()`; legacy-записи с `chipId: -1` само-лечатся в `setPlayerChips()` / `addPlayerChip()` через `_healPlayerChipEntry()`, поэтому старые сейвы мигрируют без schema bump: [src/ui/hangarChipsUI.js](../../src/ui/hangarChipsUI.js#L1506-L1532), [src/ui/hangarChipsUI.js](../../src/ui/hangarChipsUI.js#L2215-L2245).
