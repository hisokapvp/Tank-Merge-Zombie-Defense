...
- Responsive shell routing + talents header-action unification → [shouldUseFullscreenShell()](../../src/ui/supercomputerMenu.js#L414-L446), [syncResponsiveShellState()](../../src/ui/supercomputerMenu.js#L435-L446), [applySharedTalentModalClass()](../../src/ui/supercomputerMenu.js#L529-L585).
...
- `shouldUseFullscreenShell()` — canonical gate для SC-family fullscreen shell under coarse-pointer / `< 1280px`; именно этот runtime ставит `levelModal--fullscreenShell`, `scModal--fullscreenShell` и `talentTreeModal--fullscreenShell`, а CSS уже только реализует scroll/size contract. Не дублировать локальные mobile решения на уровнях отдельных overlays: [src/ui/supercomputerMenu.js](../../src/ui/supercomputerMenu.js#L414-L446), [style.css](../../style.css#L7918-L8002).
- `applySharedTalentModalClass()` обязан держать shared right-side header-actions contract для talents shell: wrapper `#supercomputerTalentsHeaderActions` создаётся/реиспользуется как `scModal__headerActions`, close получает SC-family skin и font-floor ignore, help и close принудительно живут в одном header row с порядком `help -> close`. Это deliberate унификация talents header с остальными SC shells, а не cosmetic DOM shuffle: [src/ui/supercomputerMenu.js](../../src/ui/supercomputerMenu.js#L529-L585), [style.css](../../style.css#L2682-L2720).
- `applySharedTalentModalClass()` обязан навешивать на `talentOverlay` не только `.scModal`, но и `scModal__close` + `data-font-floor-ignore="true"` на `.modalClose`, чтобы крестик дерева улучшений был визуально и по hit-area идентичен supercomputer modal: [src/ui/supercomputerMenu.js](../../src/ui/supercomputerMenu.js#L529-L585), [style.css](../../style.css#L1828-L1920).
...
| `shouldUseFullscreenShell()`, `syncResponsiveShellState()` | [src/ui/supercomputerMenu.js](../../src/ui/supercomputerMenu.js#L414-L446) | Канонический responsive/fullscreen routing для talents / hangar / tank-wall overlays |
| `setBodyScrollLock()`, `applyLayoutTuningVars()`, `applySharedTalentModalClass()` | [src/ui/supercomputerMenu.js](../../src/ui/supercomputerMenu.js#L362-L385), [src/ui/supercomputerMenu.js](../../src/ui/supercomputerMenu.js#L529-L585) | Body-lock, CSS vars, shared modal class и talents header-action unification |
...
- [src/ui/supercomputerMenu.js](../../src/ui/supercomputerMenu.js#L414-L446) — fullscreen shell routing для coarse-pointer / `< 1280px`.
- [src/ui/supercomputerMenu.js](../../src/ui/supercomputerMenu.js#L529-L585) — shared talent modal class, header-action wrapper и help/close order.
