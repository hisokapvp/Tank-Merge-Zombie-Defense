...
- Talent tree visual contract держится в `style.css`: `talentTreeContainer` задаёт `--talent-node-icon-size: 40px`, уменьшенный `--talent-row-gap`, базовые рёбра остаются явно серыми, а anchor линий считается от центра `.talentNodeIcon`; renderer рисует прямые center-to-center рёбра через `buildEdgePath()` без elbow fallback. Green glow/pulse разрешён только для `ready/active` edges от уже прокачанных талантов, причём `active` edge использует pulse + jitter без travelling dash/particle flow. Locked node остаётся читаемым и получает нейтральный overlay через `.talentNode.locked::before`, applied-node в `#talentOverlay` остаётся зелёным, а `maxed` добавляет orange overlay только на внутренний icon-shell: [style.css](../../style.css#L2076-L2108), [style.css](../../style.css#L2134-L2168), [style.css](../../style.css#L2181-L2241), [style.css](../../style.css#L2263-L2350), [src/ui/talentOverlayRenderer.js](../../src/ui/talentOverlayRenderer.js#L19-L42), [src/ui/talentOverlayRenderer.js](../../src/ui/talentOverlayRenderer.js#L91-L151), [src/ui/talentOverlayRenderer.js](../../src/ui/talentOverlayRenderer.js#L239-L239).
...
- Storage modal использует отдельный header-grid `.plStorage__header` (`44px / 1fr / 44px`) с left help button, центрированным title и close справа; grid ячеек остаётся centred, filled-cell level badge — plain text у нижней кромки, empty slots не получают placeholder glyph, а drag preview живёт отдельным fixed-layer `.plStorage__dragPreview` поверх body: [style.css](../../style.css#L6961-L7090).
...
- [style.css](../../style.css#L6961-L7000) — production line storage header/help/grid alignment.
- [style.css](../../style.css#L7001-L7090) — production line storage cells, level badge и drag preview.
