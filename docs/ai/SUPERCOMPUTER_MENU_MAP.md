...
- `shouldUseFullscreenShell()` — canonical gate для SC-family fullscreen shell under coarse-pointer / `< 1280px`; именно этот runtime ставит `levelModal--fullscreenShell`, `scModal--fullscreenShell` и `talentTreeModal--fullscreenShell`, а CSS уже только реализует scroll/size contract. Не дублировать локальные mobile решения на уровнях отдельных overlays: [src/ui/supercomputerMenu.js](../../src/ui/supercomputerMenu.js#L422-L469), [style.css](../../style.css#L7995-L8107).
