...
- `stepZombies()` держит per-zombie `_unstickTimer` / `_unstickCheckR`: если зомби не продвинулся ≥ 2px к центру за 4 сек, scalar nudge `min(8, max(1, (r - fenceR)*0.15))` подталкивает его к fence. Не срабатывает для dying/breached зомби.
- Основной механизм против застревания в decor — **decor collision pulse**, а не телепорт. `Game.DecorCollisionPulse` (`src/mechanics/decorCollision.js`) держит таймлайн: каждые `periodSec` (по умолчанию 10 сек) коллизия ВСЕХ `isWall`-декораций выключается на случайные `offDurationMinSec..offDurationMaxSec` (1–2 сек). Пока окно открыто, `isDecorCollisionSuppressed()` возвращает `true`, и оба seam'а — `resolveZombieWallMove()` и `isZombieDecorBlockedAt()` — игнорируют `state.wallDecors`, так что застрявшие зомби проходят сквозь элементы карты. Планирование окна идёт от текущего времени, чтобы после закрытия окна выдержать полный период.
- Таймлайн продвигается только явным `stepDecorCollisionPulse(dt)` внутри `stepZombies()`, поэтому пауза симуляции (menu/tab/critical) автоматически замораживает окно (loop не тикает `effDt` в paused-кадре). Kill-switch — `collisionPulse.enabled = false` в `assets/decor.json` (или `BAL.decorCollisionPulse = { enabled: false }`).
