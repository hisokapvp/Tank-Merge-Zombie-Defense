...
- Talents v2 UI (overlay + HUD активок): `game.js` (`ensureTalentUI`, `updateTalentUI`, `updateStageAbilitySlots`) + контракт в `docs/ui_talents_v2.md`.
	...
	- Talent overlay visual contract: между колонками/рядами держим увеличенные gaps, связи читаются как постоянный SVG-layer, а upgraded/ready edges получают более заметную толщину и мягкую wobble-анимацию без layout shift и без появления второго scrollbar.
	- Green applied-state рамка у talent node должна оставаться скруглённой и на node shell, и на внутренней icon-shell; canonical rounded highlight живёт в CSS/pseudo-element, а не в bitmap-иконке.
...
- Правая колонка underground hangar теперь содержит отдельную квадратную CTA `transferAll` с up-arrow icon и copy `ughTransferToUpper`; кнопка переносит всю доступную технику из underground storage в upper hangar через `onTransferAllToUpperHangar`, но disabled-state остаётся допустимым, если сверху нет свободного места.
