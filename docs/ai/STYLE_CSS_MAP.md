...
> `style.css` — CSS-монолит проекта (5076 строк) и один из главных hotspot-файлов.
...
- Hangar chips / workshop / tech unlock → [style.css](../../style.css#L3063-L4327).
- Chip craft / future chip / storage modal → [style.css](../../style.css#L4335-L5076).
...
- Игровые close-кнопки используют общий визуальный язык wasteland UI: `.levelModal__close`, `.modalClose`, `.scModal__close` и craft-slot remove `chipCraftSlotRemove` не должны превращаться в plain red circles. `scModal__close` теперь наследует тот же 44×44 hit-area и крупный крест, но перекрашивает его в green SC-skin; `talentOverlay .modalClose` обязан выглядеть один в один как SC modal close: [style.css](../../style.css#L1306-L1393), [style.css](../../style.css#L1828-L1920), [style.css](../../style.css#L4599-L4652).
...
- `techModal__rateLine`, `techModal__selectionInfo`, `techAccelChip--disabled::after` и `techAccelDustControls*` — CSS-контракт новой accel-модалки: он обязан одновременно показывать ставки `dust/chip/fragment`, остаток бюджета до cap `95%`, badge `Лимит` и `+/-` stepper для кремниевой пыли: [style.css](../../style.css#L4104-L4327).
...
| Unified button behavior / merge popup / lesson progress / hangar core / workshop / tech unlock | [style.css](../../style.css#L2666-L4334) | Shared behavior layer, merge popup, hangar chips base, workshop, tech unlock |
| Chip craft / reagent row / production line storage modal | [style.css](../../style.css#L4335-L5076) | Inventory, slots, result preview, future chip frame, storage modal |

## Hotspots
- [style.css](../../style.css#L54-L68) — CRT/grain overlay и `body.pl-storage-open`.
- [style.css](../../style.css#L1359-L1393) — общий close-skin `scModal__close`.
- [style.css](../../style.css#L1828-L1920) — базовый `modalClose` и talent-tree / SC-вариант того же close-pattern.
...
- [style.css](../../style.css#L4104-L4327) — tech accel card shell, limit badges и dust controls.
