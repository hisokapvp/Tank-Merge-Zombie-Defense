...
- До брича стратегический курс ведётся к ближайшему открытому пролому только своей текущей стороны (side-lock).
- После брича движение к `supercomputer` выполняется в world-space по прямому вектору от текущей позиции зомби (без полярного обхода по дуге).
...
- После прорыва внутренний радиальный clamp не применяется (нет «невидимого круглого забора» внутри периметра).
- После прорыва `targetR` синхронизируется с радиусом supercomputer относительно центра сцены, чтобы группа не «съезжала» в геометрический центр ангара.
- Коллизии применяются только к целым fence-сегментам и `decor`-объектам с `isWall=true` (`state.wallDecors`).
- Дороги, ангар, supercomputer и прочие внутренние элементы без `isWall` не блокируют движение зомби.
