...
- Root modal-family tokens / storage responsive token families → [style.css](../../style.css#L15-L82), [style.css](../../style.css#L8113-L8185).
...
- Root token block рядом с `--uiModalPad` и help-button offsets также владеет `--pl-storage-*` families для `default / expanded / mobile-fit`; runtime не должен хардкодить storage paddings, grid width или confirm sizing напрямую, а только переключать token set через `.plStorage__panel--expanded` / `.plStorage__panel--mobileFit`: [style.css](../../style.css#L19-L82), [src/ui/productionLineUI.js](../../src/ui/productionLineUI.js#L94-L120).
...
- `.hangarChipsModal` больше не держит локальный scale-cap: `--modsHangarScale` теперь напрямую читает master `--ui-scale`, а help-button/section paddings обязаны оставаться var-driven. Не возвращать локальный clamp до `1`, иначе modal-family снова расходится с HUD/tutorial/terminal contract: [style.css](../../style.css#L4242-L4268).
- Master `--ui-scale` contract начинается в root vars и обязан единообразно питать terminal shell, HUD, tutorial, notification/help/confirm families и dynamic modal shells. Close/help hit-area остаётся не меньше `44×44`, а font floor `12px` и drag threshold `6px` не должны «чиниться» через отдельные CSS scale hacks: [style.css](../../style.css#L15-L22), [style.css](../../style.css#L1664-L1692), [style.css](../../style.css#L3774-L3888), [style.css](../../style.css#L7612-L7726), [src/ui/fontFloor.js](../../src/ui/fontFloor.js#L22-L133).
...
- Narrow/mobile storage path использует split responsive shell contract: `.plStorage--expanded` / `.plStorage__panel--expanded` обслуживают coarse-pointer и viewports `< 1280px`, а `.plStorage--mobileFit` / `.plStorage__panel--mobileFit` — компактный `< 1000px` fit-mode с отдельными token families. Не откатывать storage modal к базовому compact shell и не обходить эти классы ad-hoc media overrides, иначе панель снова расходится с SC-family overlays: [style.css](../../style.css#L8113-L8185), [src/ui/productionLineUI.js](../../src/ui/productionLineUI.js#L94-L120), [index.html](../../index.html#L255-L270).
...
- `scModal--fullscreenShell` и `talentTreeModal--fullscreenShell` — общий SC-family fullscreen contract для coarse-pointer и viewports `< 1280px`: width/max-height/body scroll containment/embedded talents mount документируются здесь, а не как локальные mobile overrides отдельных модалок: [style.css](../../style.css#L7995-L8107), [src/ui/supercomputerMenu.js](../../src/ui/supercomputerMenu.js#L422-L469).

## Оглавление файла
| Блок | Строки | Назначение |
|---|---|---|
| Theme vars / body / HUD / stage shell | [style.css](../../style.css#L1-L420) | CSS-переменные (включая root-tile sizing vars и `--ui-scale:1` default), safe-area, top-right terminal shell, narrow-screen terminal header, stage layout |
...
| Adaptive UI scaling (`--ui-scale`) | [style.css](../../style.css#L15-L82), [style.css](../../style.css#L1664-L1692), [style.css](../../style.css#L3774-L3888), [style.css](../../style.css#L4242-L4268), [style.css](../../style.css#L7612-L7726), [style.css](../../style.css#L8113-L8185) | Root vars + HUD/tutorial/hangar/shared-shell blocks: modal/storage/help/confirm/tooltip/notification/terminal families масштабируются от master token; storage responsive shell меняет только token families `default/expanded/mobile-fit`, close-buttons **NOT** scaled — 44×44 hit-area сохраняется |
...
- [style.css](../../style.css#L7455-L7565) — production line storage cells, level badge и drag preview.
- [style.css](../../style.css#L7420-L7453) — production line storage header right-actions wrapper, help/close grouping и title padding.
- [style.css](../../style.css#L15-L82) — root modal-family tokens, включая `--uiModalPad`, help-button offsets и storage `default/expanded/mobile-fit` token families.
- [style.css](../../style.css#L7995-L8107) — fullscreen shell contract для SC/talents narrow-mobile overlays.
- [style.css](../../style.css#L8113-L8185) — Production Storage responsive shell classes: safe-area wrapper, expanded/mobile-fit token swaps, enlarged or compacted grid/confirm shell.
- [style.css](../../style.css#L7701-L7725) — scale-aware wrappers `.modsTankWall__panelActions` / `.modsTankWall__footerActions`.
- [style.css](../../style.css#L15-L82), [style.css](../../style.css#L1664-L1692), [style.css](../../style.css#L3774-L3888), [style.css](../../style.css#L4242-L4268), [style.css](../../style.css#L7612-L7726) — master adaptive UI scaling (`--ui-scale`): root tokens, HUD, tutorial, hangar modal passthrough, shared shell selectors и storage token families; JS floor задаётся в `game.js` как `0.40`, а cap остаётся `1.0` на 1920×1080 и выше.
