...
- Narrow/mobile storage path использует отдельный expanded shell contract: `.plStorage--expanded` растягивает overlay по safe-area, а `.plStorage__panel--expanded` поднимает shell/cell scale, header paddings, grid width и confirm sizing. Не откатывать storage modal к базовому compact shell на `< 1280px`, иначе панель снова визуально схлопывается относительно SC-family overlays: [style.css](../../style.css#L8008-L8063), [index.html](../../index.html#L255-L270).
...
- `scModal--fullscreenShell` и `talentTreeModal--fullscreenShell` — общий SC-family fullscreen contract для coarse-pointer и viewports `< 1280px`: width/max-height/body scroll containment/embedded talents mount документируются здесь, а не как локальные mobile overrides отдельных модалок: [style.css](../../style.css#L7918-L8002), [src/ui/supercomputerMenu.js](../../src/ui/supercomputerMenu.js#L414-L446).
...
- [style.css](../../style.css#L7918-L8002) — fullscreen shell contract для SC/talents narrow-mobile overlays.
- [style.css](../../style.css#L8008-L8063) — expanded Production Storage shell для mobile/<1280: safe-area wrapper, scale vars, enlarged grid/confirm shell.
