...
## Cache-bust contract
- После merged reset/cooldown update `index.html` держит не один общий token, а согласованные query-группы: `style.css`, `src/i18n/fallbackStrings.js`, `src/ui/modals.js` и `game.js` используют `?v=20260322-branch2-talent-reset-cooldown`, `src/ui/talentOverlayRenderer.js` остаётся на `?v=20260322-branch1-talents-storage-v2`, а `src/ui/supercomputerMenu.js` и `src/ui/productionLineUI.js` — на `?v=20260322-ui-postmerge`. При правках DOM/CSS/i18n/runtime для talent reset/storage flow нужно bump-ать затронутую группу целиком, иначе браузер смешает новый shell и старую modal-логику: [index.html](../../../index.html#L9-L10), [index.html](../../../index.html#L606-L621), [index.html](../../../index.html#L648-L648).
