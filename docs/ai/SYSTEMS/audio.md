...
- Позиция воспроизведения: трек, ушедший в тишину (меню/пауза), **ставится на паузу на месте** — `currentTime` не сбрасывается. Открытие любого меню/модалки/суперкомпьютера ставит боевой трек на паузу там, где он играл, а закрытие последнего меню возобновляет его с той же позиции. Правило одинаково для `battleCalm` (без волны) и `battleWave` (во время волны атаки). Rewind (`currentTime = 0`) выполняется только при реальной смене источника/явном reset. `getState().tracks[id]` дополнительно отдаёт `paused` и `currentTime` для верификации.
