...
- Базовые статы (HP/armor) для текущего уровня берутся из `levels[level-1].segmentMaxHp` и `levels[level-1].armorFlat`. Текущий HP считается компаундингом `round(base * FENCE_HP_MUL^applied)` (`FENCE_HP_MUL` сорсится из `getWallHpCompoundFactor()`, `{kind:'mulCompound', factor:1.01}`). Текущая броня (solo-pipeline-yandex-vk#2 item 4) считается **итеративным** компаундингом +1% с floor +1 ед./шаг через `computeWallArmorCompound()` (источник `getStatGrowth('walls','armorFlat') = {factor:1.01, floor:1}`); прежняя константа `FENCE_ARMOR_MUL` удалена.
