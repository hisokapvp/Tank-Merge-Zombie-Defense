...
- Проломы fence:
	...
	- поиск ближайшего пролома для steer выполняется только на текущей стороне зомби (side-local), без глобального fallback по всем сторонам.
	- если на текущей стороне уже есть пролом, steer к пролому не ограничивается awareness-радиусом.
	- пока на стороне есть пролом и зомби ещё не `breached`, атака целых сегментов на этой стороне не выбирается как attack-target.
	- fallback по всем сторонам допустим только для локального `hit-test` в самой точке касания дыры (corner-case у углов).
