...
- Под-вкладки: «Улучшение чипов» (`workshopTabChipUpgrade`, панель `workshopPanelChipUpgrade`) и «Открытие технологий» (`workshopTabTechUnlock`, панель `workshopPanelTechUnlock`).
...
### Правила размещения
- Установка чипа: сначала нужно нажать на слот (треугольник), затем на чип в списке. Авто-установка отключена. При клике на чип без выбранного слота показывается toast «Сначала выберите слот».
...
### QA (ручной)
- Суперкомпьютер → Модификации ангара: открываются 2 вкладки, «Мастерская» работает с под-вкладками «Улучшение чипов» и «Открытие технологий».
...
- «Улучшение чипов»: кнопка «Объединить» удалена; merge через drag-and-drop (зажать чип и перетащить на одноименный).
