...
| `playerChips` | hangar | `Game.HangarChipsUI.getPlayerChips()` | [src/persistence/storage.js](../../../src/persistence/storage.js) `serializePlayerChips()`; writer модуль — [src/ui/hangarChipsUI.js](../../../src/ui/hangarChipsUI.js) | `[]`; malformed/нерезолвимые записи (нет валидного `chipId` И нет `modIds`) дропаются. Reader — оба restore-пути через `HangarChipsUI.setPlayerChips(payload.slice(), {reason:'restore'})` **безусловно** (payload без поля → `[]`). Внимание: `Game.State.getPlayerChips/setPlayerChips` — задокументированный canonical owner, которого **нет** в кодовой базе, поэтому `state.playerChips` в runtime всегда `[]` и writer ОБЯЗАН читать live-инвентарь через UI API. `partial-preserve` |
...
| `techStudying` | hangar | `Game.HangarChipsUI.getTechStudying()` | [src/persistence/storage.js](../../../src/persistence/storage.js) `serializeTechStudying()`; writer модуль — [src/ui/hangarChipsUI.js](../../../src/ui/hangarChipsUI.js) | `null`; malformed/неполный объект (`modId <= 0`, `duration <= 0`) схлопывается в `null`; `elapsed` clamp `[0, duration]`, `acceleratedPct` clamp `[0, 100]`. Reader — `restoreFullState` / `applySavedProgress` через `HangarChipsUI.setTechStudying(...)` **безусловно** (payload без поля → `null`), что перезапускает таймер. `partial-preserve` |
| `techFeedProgress` | hangar | `Game.HangarChipsUI.getTechFeedProgress()` | [src/persistence/storage.js](../../../src/persistence/storage.js) `serializeTechFeedProgress()`; writer модуль — [src/ui/hangarChipsUI.js](../../../src/ui/hangarChipsUI.js) | `{}`; ключи — modId, значения — скармливаемые чипы; невалидные/`<= 0` записи дропаются. Reader — оба restore-пути через `HangarChipsUI.setTechFeedProgress(...)` **безусловно** (payload без поля → `{}`) |
| `undergroundHangar` | hangar | `state.undergroundHangar` | [src/persistence/storage.js](../../../src/persistence/storage.js) `serializeUndergroundHangar()` | `null`/отсутствует → пустой ангар (`{ cells: [] }`); 16 ячеек, каждая хранит либо `tank`, либо `drone` (никогда оба). Дроны попадают сюда, когда все 9 основных слотов заняты (`Drones.addDron()` → `UndergroundHangar.storeDrone()`), поэтому без этого поля overflow-дроны исчезали после reload. Reader — `restoreFullState()` / `applySavedProgress()` **безусловно** (payload без поля очищает ангар, а не утекает прошлую сессию). `partial-preserve` (входит в `takeProgressSnapshot`/`restoreProgressSnapshot`) |
| `siliconDust` | hangar | `Game.HangarChipsUI.getSiliconDust()` | [src/persistence/storage.js](../../../src/persistence/storage.js) `serializeSiliconDust()`; writer модуль — [src/ui/hangarChipsUI.js](../../../src/ui/hangarChipsUI.js) | `0`; отрицательные/нечисловые значения clamp'ятся в `0`. Reader — оба restore-пути через `HangarChipsUI.setSiliconDust(...)` **безусловно** (payload без поля → `0`) — нейтральная запись, НЕ инкрементит `dustEarnedLifetime` (для этого есть `creditSiliconDust`) |
...
| `stats` | progression | `state.stats` | [game.js](../../../game.js) (`manualFenceRepairsCount`, `modifierTechUnlocksCount`, `droneAcquisitionsCount`, `noRepairAttackWaveStreakCount`, `currentWaveCount`) | `stats.*Count` canonical, `achievements.total*` — legacy fallback. `currentWaveCount` — per-run (нет legacy mirror): не входит в `takeProgressSnapshot()`, поэтому partial reset и New Game начинают с 0 |
...
| `supercomputer` | progression | `state.supercomputer` | [game.js](../../../game.js) supercomputer flow | `computerLevel = 0` baseline для new game. **`x`/`y`/`offsetY` — runtime-derived**: восстанавливаются из payload, но сразу перезаписываются `repositionSupercomputerFromBoard()` (центр `boardRect` по X, низ `boardRect` + `offsetY` по Y), потому что зависят от размера viewport. Сейв, сделанный в fullscreen, не должен прибивать суперкомпьютер к fullscreen-координатам при загрузке в оконном режиме. **`levelKills`** — альтернативный per-level счётчик убийств (порог `LEVEL_KILL_THRESHOLD = 500000`): НЕ равен HUD-счётчику `state.kills`, сбрасывается в 0 после любого полученного уровня (и по XP, и по убийствам), участвует в partial-reset snapshot рядом с `xp` ([src/mechanics/levelFlow.js](../../../src/mechanics/levelFlow.js), [src/core/worldReset.js](../../../src/core/worldReset.js)) |
| `boostUntil`, `activeEffects`, `timedEffectsRemainingSec` | world | `state.boostUntil`, `state.activeEffects` через `Game.getTimedEffectRemainders()` | [src/persistence/storage.js](../../../src/persistence/storage.js) `serializeTimedEffectRemainders()`, [game.js](../../../game.js) `restoreTimedEffectsFromSave()` | `0` / `{}` / `null`. **Относительные остатки** timed-эффектов: `boostSec` (speed-буст суперкомпьютера), `attackSec` (Шквал), `defenseSec` (Купол), `economySec` (Золотое время). Absolute `boostUntil` / `activeEffects.*Until` живут в домене `nowSec()`, который перезапускается с ~0 на каждой загрузке, поэтому сырое значение давало бафф длительностью «сколько шла прошлая сессия» (баг «Шквал 1100+ секунд после загрузки»). Reader предпочитает `timedEffectsRemainingSec`; legacy-поля трактует как остатки и клампит до полной длительности эффекта. Runtime self-healing guard (`clampTimedEffectsToTimeDomain()` внутри `normalizeActiveEffectsTimestamps()`, каждый кадр) снимает poisoned-значения даже без reload |
...
| `nextCrateAt`, `crateRemainingSec` | world | `state.nextCrateAt` (live) через `Game.getCrateRemainingSec()` | [src/persistence/storage.js](../../../src/persistence/storage.js) `serializeCrateRemainingSec()`, [game.js](../../../game.js) `restoreFullState()` | `null` / полный `BAL.crateIntervalSec`. **`crateRemainingSec` — относительный** остаток таймера подарочного бокса: `state.nextCrateAt` живёт в домене `nowSec()` (session-relative, сбрасывается в ~0 на каждой загрузке), поэтому сырой absolute timestamp в payload непригоден (сохранение на 60-й секунде → после reload `nextCrateAt = 90` при `nowSec() ≈ 3` → `maybeSpawnCrate()` никогда не срабатывает, боксы перестают падать). Legacy `nextCrateAt` читатель трактует как остаток и клампит до `[0, crateIntervalSec]`. Тот же класс бага/фикса, что у attack-wave |
| `boostUntil`, `mapSeeds` | world | `state.*` | [game.js](../../../game.js), world init | сохраняются если присутствуют |
| `attackWaveRemainingSec`, `attackWaveActive`, `attackWaveRemainingActiveSec` | world | live `worldEventsState.attackStartAt`/`currentAttackStartAt`/`attackEndAt` через `Game.getAttackWaveSnapshot()`; fallback `state.attackWave*` | [src/systems/worldEventsRuntime.js](../../../src/systems/worldEventsRuntime.js) (reader/writer seam), [src/persistence/storage.js](../../../src/persistence/storage.js) `serializeAttackWaveSnapshot()`, [game.js](../../../game.js) `restoreFullState()` → `applyLoadedAttackWaveSnapshot()` | `null` / `false` / `0`; clamped `[0, attackEverySec]` и `[0, attackDurationSec]`. **Относительные** остатки: absolute `attackStartAt`/`currentAttackStartAt`/`attackEndAt` не персистятся. `attackWaveActive: true` = игрок сохранился ВНУТРИ волны → загрузка продолжает волну с сохранённым остатком (без флага волна выключалась и заново отсчитывала полный `attackEverySec`). Обычная загрузка продолжает расписание; `forceFenceRuntimeResetOnLoad` (= «Перезапуск симуляции» / critical-сейв) игнорирует поля и даёт полный `attackEverySec` через `scheduleFirstAttackWaveAfterRestart()`; legacy payload без полей — тоже полный интервал |
...
### Регрессионный anchor для module-owned hangar-ресурсов

Кремниевая пыль, незавершённое таймерное изучение технологии и per-tech fed-chip counters живут в module-owned состоянии `src/ui/hangarChipsUI.js`, а НЕ в `state`, поэтому каждый из них требует писателя в `serializeState()` и **безусловного** чтения в обоих restore-путях. Регрессия закреплена тестом `DSP-1..12` в [Test/pack18/hangarResourcePersistence.test.js](../../../Test/pack18/hangarResourcePersistence.test.js) (round-trip через реальный write-path `saveSlot`/`loadSlot`, New Game очистка, нейтральный dust-seam, known keys, schema, entry token). Аналогичный контракт для фрагментов — `FGP-1..8` в [Test/pack12/playerFragmentsPersistence.test.js](../../../Test/pack12/playerFragmentsPersistence.test.js).

Тот же класс дефекта для целых чипов закреплён тестом `CPS-1..10` в [Test/pack19/playerChipsPersistence.test.js](../../../Test/pack19/playerChipsPersistence.test.js). Причина рецидива: документальный canonical owner `Game.State.getPlayerChips/setPlayerChips` **не существует**, поэтому инвентарь живёт в module-owned `_playerChipsFallback`, а `state.playerChips` навсегда остаётся `[]`. Любой writer, читающий `state.playerChips`, молча сохраняет пустой массив — ровно поэтому баг возвращался при рефакторингах.

### TUT-8R..TUT-8Z — regression pack anchor
...
Anchor-кейсы для module-owned hangar-ресурсов: `TUT-8Y` (silicon dust / tech study / feed progress) и `TUT-8Z` (whole-chip inventory, live-first writer) в [Test/pack4/tutorial_first_run_runtime.test.js](../../../Test/pack4/tutorial_first_run_runtime.test.js). Следующий свободный slot — `TUT-8AA`.
...
- `stats.currentWaveCount` — **per-run** счётчик завершённых волн атаки для HUD-панели `currentWaveText` («Текущая волна: X»). Инкремент ровно один раз на переход attack active → inactive в `finalizeNoRepairAttackWaveEpisode()` через `incrementCurrentWaveCounter()`; `beginNoRepairAttackWaveEpisode()` счётчик не трогает. Поле намеренно **не** входит в `takeProgressSnapshot()`/`restoreProgressSnapshot()`, поэтому «Перезагрузка симуляции» и New Game начинают с 0, а save/load внутри одного run прогресс сохраняет: [game.js](../../../game.js), [src/persistence/initialState.js](../../../src/persistence/initialState.js), [src/persistence/storage.js](../../../src/persistence/storage.js)
...
- **New Game обязан очищать инвентарь чипов без перезагрузки страницы.** `resetGameState()` пересоздаёт `state` через `createInitialState({ reason: 'new_game' })`, поэтому `state.playerChips` становится `[]`. Но `HangarChipsUI` владеет собственными копиями chips/fragments/silicon-dust/tech-study, которые НЕ являются derived view от `state` и без явного сброса переживали New Game до перезагрузки страницы.
  - Канонический seam: `Game.HangarChipsUI.resetPlayerInventory(meta)` — очищает `playerChips` (через `setPlayerChips([], { reason: 'new_game' })`, один emit `playerChips.changed`), `_playerFragments`, `_siliconDust`, `_techFeedProgress`, останавливает `_techStudying` + его timer и вызывает `resetTransientUiState()`.
  - **Установленные в ячейки чипы тоже обязаны сбрасываться.** Они живут не в `state`, а в module-owned grid `_cells` внутри `hangarChipsUI.js` (`Game.HangarChips.createHangarCellsState()`, 16 ячеек с `redSlots`/`yellowSlots`/`activeModifiers`). Этот grid не сериализуется и не входит в `createInitialState()`, поэтому без явного сброса чипы, вставленные в ячейки, переживали New Game. Seam делает `_cells = null` + `ensureCells()`, дополнительно гасит залипший `_chipFilter`/`_selectedSlot`/`_activeSlotActions`, сбрасывает `_selectedCell = 0` и отменяет in-flight drag через зарегистрированный `_cancelDragsHook`.
  - Вызывается ровно в ветке `reason === 'new_game'` внутри `resetGameState()`: [game.js](../../../game.js).
