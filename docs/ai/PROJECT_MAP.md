...
| Combat cadence и live TTK теперь canonical через asset stats: `tankStats()` читает `assets/tanks.json.stats.baseDamage/attackSpeed`, Balance Lab shared kernel и editor fallback зеркалят этот же контракт, а legacy `BAL.dmgMultPerLevel`, `fireRateBase`, `fireRateAddPerLevel`, `zombieHpBase`, `zombieHpExtraPerLevel` зафиксированы на `x1` и locked в registry, поэтому баланс меняют через `series.tank.baseDamage` и `series.zombie.health`, а не через runtime-кривые. | [game.js](../../game.js#L5046-L5078), [tools/balance-shared.js](../../tools/balance-shared.js#L26-L39), [tools/balance-shared.js](../../tools/balance-shared.js#L459-L487), [tools/balance-registry.js](../../tools/balance-registry.js#L472-L584), [tools/balance-editor.html](../../tools/balance-editor.html#L503-L513) |
| Для lingering chip mods `10..14` флаг `effect.enabled` в `assets/chips.json` отключает только code-drawn fallback visual. `applyImpactEffects()` всё равно создаёт gameplay pools/nodes/marks и сохраняет `effectSprite`; render-path читает `codeVisualEnabled` через decal/node/mark state, а не по отсутствию runtime-объекта. | [src/mechanics/chipEffects.js](../../src/mechanics/chipEffects.js#L876-L1015), [game.js](../../game.js#L8799-L8812), [game.js](../../game.js#L14151-L14186) |
| Если зомби застрял в decor после спавна и за 20 секунд не добрался до fence, runtime обязан один раз телепортировать его к внешней стороне забора на 20–30 px отступа, не за fence и с обходом blocked landing point; `zombieFenceLimit()` при этом остаётся test-safe и умеет fallback'иться без внешних helpers. | [game.js](../../game.js#L6734-L6778), [game.js](../../game.js#L7672-L7718), [game.js](../../game.js#L7826-L7997) |
...
| Procedural `auraOrbs` для `drawTankAura()` читаются из normalized `TankSprites.config.auraOrbs`; свежесть достигается через `TankSprites.refreshConfig()` / `reloadConfig()` вне render hot path, а `drawTankAura()` только читает cached config и safe defaults. Некорректные `auraOrbs` значения нормализуются loader'ом до fallback shape. | [assets/tanks.json](../../assets/tanks.json#L1-L17), [src/render/spriteLoaders.js](../../src/render/spriteLoaders.js#L233-L258), [src/render/spriteLoaders.js](../../src/render/spriteLoaders.js#L617-L644), [game.js](../../game.js#L14199-L14297) |
| `Game.Events` — async EventBus с payload-by-id contract и rAF-coalescing: state/runtime writers emit topics после успешной mutation, UI/render subscribers только schedule repaint/effects и не мутируют state из paint path. Craft path intentionally emits both `playerChips.changed reason='craft'` and `chips.crafted`; subscriber-side dedup обязателен при двойной подписке. | [src/core/events.js](../../src/core/events.js#L1-L120), [src/ui/hangarChipsUI.js](../../src/ui/hangarChipsUI.js#L1399-L1457), [SYSTEMS/events.md](SYSTEMS/events.md) |
| `HudScratch.acquireArray(ownerTag, subSlot)` идемпотентен per frame: повторный вызов для той же пары возвращает тот же stable array lease без allocation и допускает re-entry. API не является unbounded allocator; frame boundary очищает leases через `beginFrame()`. | [src/render/hudScratch.js](../../src/render/hudScratch.js#L1-L246), [game.js](../../game.js#L12419-L12428), [SYSTEMS/hud_scratch.md](SYSTEMS/hud_scratch.md) |
...
| `index.html` обязан подключать `src/ui/fontFloor.js`; модуль держит глобальный floor `10px` для DOM и canvas-текста, восстанавливает исходный inline `font-size`/priority, экспортирует `Game.FontFloor.SKIP_SELECTORS` как canonical skip-list для unified close/remove controls и даёт узкую scheduler observability через `getSchedulerMetrics()`. | [index.html](../../index.html#L635), [src/ui/fontFloor.js](../../src/ui/fontFloor.js#L9-L17), [src/ui/fontFloor.js](../../src/ui/fontFloor.js#L61-L155), [src/ui/fontFloor.js](../../src/ui/fontFloor.js#L212-L270) |
| Close-кнопки `crate/level/modal/lesson` и SC-overlay family используют единый 44×44 X-pattern: orange-ветка живёт в `.crateModal__close`, `.levelModal__close`, `.modalClose`, `.lessonProgress__close`, green-ветка — в `scModal__close`, `#talentOverlay .modalClose`, `.modalClose.scModal__close`; font floor обязан пропускать эти селекторы и при переходе элемента в skip-path снимать собственный inline clamp, чтобы не ломать glyph, hit-area и визуальную унификацию. | [src/ui/fontFloor.js](../../src/ui/fontFloor.js#L9-L16), [src/ui/fontFloor.js](../../src/ui/fontFloor.js#L102-L117), [src/ui/fontFloor.js](../../src/ui/fontFloor.js#L133-L155), [src/ui/productionLineUI.js](../../src/ui/productionLineUI.js#L44-L61), [style.css](../../style.css#L54-L68), [style.css](../../style.css#L1042-L1091), [style.css](../../style.css#L1337-L1423), [style.css](../../style.css#L1859-L1951), [style.css](../../style.css#L3116-L3164) |
...
| Canvas/storage/workshop drag path обязан быть touch-safe и threshold-gated: touch `pointer*` вызывает `preventDefault()` только на cancelable touch events, pointer capture снимается на up/cancel, а drag state/ghost/target обновляются только после порога `6px`. Этот контракт общий для canvas tank drag, storage box drag, hangar chip drag и underground hangar modal drag; hangar chip/underground paths дополнительно слушают `pointercancel` для cleanup. | [game.js](../../game.js#L11196-L11405), [src/ui/productionLineUI.js](../../src/ui/productionLineUI.js#L150-L257), [src/ui/hangarChipsUI.js](../../src/ui/hangarChipsUI.js#L4065-L4395), [src/ui/undergroundHangarUI.js](../../src/ui/undergroundHangarUI.js#L411-L476) |
| Browser input contract deliberately suppresses native context menu на обоих runtime-путях: legacy path ставит one-shot document-level `contextmenu` guard, Phaser bootstrap зеркалит это через `disableContextMenu:true`, а canvas `#c` и underground hangar drag hosts сохраняют `touch-action:none` + `-webkit-touch-callout:none`, чтобы long-press/right-click не уводили управление в браузер. | [game.js](../../game.js#L11153-L11157), [src/phaser/phaserBootstrap.js](../../src/phaser/phaserBootstrap.js#L106-L116), [style.css](../../style.css#L1115-L1124), [style.css](../../style.css#L4744-L4760) |
| Fence repair pricing source-of-truth живёт в `Game.FenceRepair`: базовая цена резолвится config-first через `assets/fence.json -> repair.costCoinsByLevel[level] -> levels[level-1].repairCostCoins / levels[level-1].repair.costCoins -> buyTankCost(level)`, после чего `computeRepairCost(fenceLevel, repairCount)` добавляет cumulative surcharge `+repairCount × max(1, ceil(baseCost × 0.01))`. `game.js` только инициализирует модуль в boot и делегирует `getFenceRepairCostCoins()` / `tryRepairFenceSegmentAt()`. | [assets/fence.json](../../assets/fence.json#L1573-L1578), [src/mechanics/fenceRepair.js](../../src/mechanics/fenceRepair.js#L45-L74), [src/mechanics/fenceRepair.js](../../src/mechanics/fenceRepair.js#L148-L165), [game.js](../../game.js#L2278-L2285), [game.js](../../game.js#L7500-L7516), [game.js](../../game.js#L14824-L14833) |
| SC-family overlay hide path обязан быть focus-safe: перед `setOverlayOpen(false)` runtime резолвит внешний focus target через `resolveOverlayHideFocusTarget()` и уводит фокус наружу через `moveFocusOutsideOverlay()`, чтобы shared help/modal shell не оставлял activeElement внутри скрытого overlay. | [src/ui/supercomputerMenu.js](../../src/ui/supercomputerMenu.js#L219-L249) |
...
| TMZD analytics rollout остаётся consent-aware и non-blocking: remote adapters включаются только при `tmzdAnalyticsEnabled` + flags/experiment rollout, limited-event mode отсекает non-`limitedByDefault` события, а health snapshot обязан держать read-back/manual smoke/weekly review stale reasons без сетевых блокировок hot path. | [src/telemetry/telemetry.js](../../src/telemetry/telemetry.js#L1-L149), [src/telemetry/telemetry.js](../../src/telemetry/telemetry.js#L331-L517), [src/telemetry/telemetry.js](../../src/telemetry/telemetry.js#L628-L759), [src/analytics/collector.js](../../src/analytics/collector.js#L1-L372), [src/flags/flags.js](../../src/flags/flags.js#L1-L223), [src/experiments/experiments.js](../../src/experiments/experiments.js#L1-L343) |
...
| `Game.HangarChipsUI.init()` | [src/ui/hangarChipsUI.js](../../src/ui/hangarChipsUI.js#L3966-L4420) | 3966–4420 | Инициализация overlay ангара: hover + long-press tooltips, touch-safe drag-drop, pointer capture / `pointercancel` cleanup |
...
| `Game.Funnel.getAnalyticsAdoptionSnapshot()` | [src/analytics/funnel.js](../../src/analytics/funnel.js#L1-L259) | 1–259 | Analytics adoption milestones и 7-day freshness для weekly review/manual smoke |
...
| Game.Events / EventBus topics | [SYSTEMS/events.md](SYSTEMS/events.md), [../events.md](../events.md) | |
...
| HUD scratch arrays / leases | [SYSTEMS/hud_scratch.md](SYSTEMS/hud_scratch.md), [SYSTEMS/hud.md](SYSTEMS/hud.md) | |
