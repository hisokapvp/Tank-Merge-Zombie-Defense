...
| Визуал ауры танка больше не форсируется high-level sprite-уровнем: `resolveTankAuraVisual(cellIndex, level)` считает реально установленные чипы через `getInstalledChipCountForCell(cellIndex)`, активирует `aura1/aura2/aura3` по count `1..3`, а `computeAuraBand()` остаётся только fallback-path, если sprite-вариант недоступен. `TankSprites.auraVariantLevels` в loader'е — это asset lookup (`10/20/30`), а не runtime gate. | [game.js](../../game.js#L13267-L13310), [src/render/spriteLoaders.js](../../src/render/spriteLoaders.js#L441-L444), [src/render/spriteLoaders.js](../../src/render/spriteLoaders.js#L591-L610) |
