...
- Для mobile / narrow-shell path storage modal использует отдельный expanded contract: `.plStorage--expanded` растягивает overlay по safe-area, а `.plStorage__panel--expanded` увеличивает `--pl-storage-shell-scale`, `--pl-storage-cell-scale`, header paddings, grid width и confirm-box sizing. Это deliberate расширение shell под `< 1280px`, а не случайный oversized side effect: [index.html](../../../index.html#L255-L270), [style.css](../../../style.css#L8008-L8063).
