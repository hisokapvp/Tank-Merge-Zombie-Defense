...
## Фокус документации на 2026-03-24
...
- Achievements runtime: все 28 reward modes живут в единой `REWARD_TABLE` внутри `src/mechanics/achievementRewards.js`; каждая запись теперь несёт `i18nKey` для автоматического i18n-lookup описания награды в popup; `grant()` и `grantByTable()` заменяют старые if-else/switch chains; `grantAchievementUpgradePoints()` и `grantAchievementReward()` синхронизируют `TalentsV2.setFreePoints()` в трёх независимых grant-path'ах; `recordModifierTechUnlock()` всегда вызывает `recalculateUnlocks()` (фикс inference race); debug panel (`debugUnlockAchievementAndClaim`) использует canonical progress hooks; `restoreFullState`/`applySavedProgress` реконструируют `_unlockedTechs` из `completedModifierTechs`; `ensureState` **всегда** мержит inferred techs в `completedModifierTechs` (не только при пустой map — фикс потери tier-2 tech progress для legacy saves); `reconcileUnlockedTechsFromData()` в hangarChips.js восстанавливает `_unlockedTechs` из реального инвентаря чипов/ячеек/фрагментов и вызывается в обеих restore-ветках game.js; stale-episode fallback в `handleNoRepairAttackWaveTransition` ловит пропущенные finalization между кадрами; CRT/grain overlay добавлен на achievement popup через `body.achievement-popup-open`; семейство `stable_income` (5 уровней, `moneyEarned` progressType) считает суммарный заработок монет через zombie kill coins и shot coins hooks в `game.js`; `resetGameState` в режиме `new_game` очищает `_unlockedTechs` через `HangarChips.setUnlockedTechs({})`.
