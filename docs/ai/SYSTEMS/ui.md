...
- `src/ui/supercomputerMenu.js` теперь документируется как canonical owner SC-family shell contract: `showSharedHelpModal()` / `syncSharedHelpButtonCopy()` держат общий help-shell для talents/hangar/tank-wall/storage, `shouldUseFullscreenShell()` остаётся единственным fullscreen helper для coarse-pointer / `< 1280px`, а `applySharedTalentModalClass()` владеет talents header action-row и видимым порядком `help -> close`; embedded `.modalClose` сохраняется только как parity-skin/hit-area слой: [src/ui/supercomputerMenu.js](../../../src/ui/supercomputerMenu.js#L150-L185), [src/ui/supercomputerMenu.js](../../../src/ui/supercomputerMenu.js#L422-L469), [src/ui/supercomputerMenu.js](../../../src/ui/supercomputerMenu.js#L553-L690), [style.css](../../../style.css#L2682-L2720), [style.css](../../../style.css#L7995-L8107).
- Production Storage shell теперь зафиксирован как `default / expanded / mobile-fit` contract: `_getResponsiveShellMode()` выбирает `mobile-fit` ниже `1000px`, `expanded` для coarse-pointer или `< 1280px`, а `style.css` root token block рядом с modal-family vars хранит `--pl-storage-*` default/expanded/mobile families, которые переключаются только через `.plStorage__panel--expanded` / `.plStorage__panel--mobileFit`: [src/ui/productionLineUI.js](../../../src/ui/productionLineUI.js#L94-L120), [index.html](../../../index.html#L255-L262), [style.css](../../../style.css#L19-L82), [style.css](../../../style.css#L8113-L8185).
...
- Для narrow/mobile storage path modal использует split responsive contract: `_getResponsiveShellMode()` переключает `default / expanded / mobile-fit`, `.plStorage--expanded` растягивает overlay по safe-area для coarse-pointer / `< 1280px`, а `.plStorage--mobileFit` включает отдельный `< 1000px` fit-mode с собственными token families. Реализация deliberate остаётся в runtime class toggles + root `--pl-storage-*` vars, а не в ad-hoc inline/mobile overrides: [src/ui/productionLineUI.js](../../../src/ui/productionLineUI.js#L94-L120), [style.css](../../../style.css#L19-L82), [style.css](../../../style.css#L8113-L8185), [index.html](../../../index.html#L255-L270).
