...
- Banner «Атака началась» теперь читает строки через `t()`:
	...
	- Номер волны для `{wave}` берётся из helper `getCurrentAttackWaveNumber()` ([game.js](../../../game.js)) — это `state.stats.currentWaveCount + 1`, тот же per-run источник, что читает HUD-панель «Текущая волна: X». Lifetime-счётчик `state.stats.attackWavesCompletedCount` для баннера использовать нельзя: он не входит в save payload и после загрузки равен 0, из-за чего баннер всегда начинал с «1 волна атаки» (см. CHANGELOG 2026-09-24).
