...
| `assets/zombies.json` держит shared `atlas`, `atlasesById` и повторяющийся `types[]` contract; `ZombieSprites.load()` нормализует `atlasPath`, preload'ит per-type atlas images и отдаёт `getAtlasImage(type, preferSharedAtlas)`, а `src/render/zombieRender.js` использует shared atlas только для `deathCommon` / `deathUsesCommonAtlas`, иначе берёт atlas конкретного типа. | [assets/zombies.json](../../assets/zombies.json#L1-L101), [assets/zombies.json](../../assets/zombies.json#L101-L2939) _(строки приблизительные, повторяющийся `types[]` block)_, [src/render/spriteLoaders.js](../../src/render/spriteLoaders.js#L222-L426), [src/render/zombieRender.js](../../src/render/zombieRender.js#L9-L29), [src/render/zombieRender.js](../../src/render/zombieRender.js#L49-L54) |
...
| `Game.SupercomputerMenu` остаётся canonical owner shared help-shell, fullscreen-shell routing и talents close/help header-actions для SC-family overlays; production storage не заводит отдельный help modal, а переиспользует этот API и переключает только runtime classes `plStorage--expanded/mobileFit`, которые питаются root `--pl-storage-*` token families в `style.css`. | [src/ui/supercomputerMenu.js](../../src/ui/supercomputerMenu.js#L150-L185), [src/ui/supercomputerMenu.js](../../src/ui/supercomputerMenu.js#L422-L469), [src/ui/supercomputerMenu.js](../../src/ui/supercomputerMenu.js#L553-L636), [src/ui/supercomputerMenu.js](../../src/ui/supercomputerMenu.js#L620-L690), [src/ui/productionLineUI.js](../../src/ui/productionLineUI.js#L94-L153), [style.css](../../style.css#L19-L82), [style.css](../../style.css#L7995-L8185), [index.html](../../index.html#L255-L262) |
