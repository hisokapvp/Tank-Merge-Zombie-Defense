...
- Зелёный glow/pulse разрешён только для outgoing edges от талантов с ненулевым applied rank: `drawBranchEdges()` помечает такие связи как `ready` или `active`, а `base` остаётся серой без ауры. Travelling dash/particle flow не используется; вместо него `applyEdgeMotion()` задаёт per-edge wobble timing, поэтому `ready/active` связи заметно shake/wobble-ятся как эффект тока/энергии и читаются сильнее после апгрейда: [src/ui/talentOverlayRenderer.js](../src/ui/talentOverlayRenderer.js#L45-L64), [src/ui/talentOverlayRenderer.js](../src/ui/talentOverlayRenderer.js#L91-L151), [style.css](../style.css#L2134-L2179).
